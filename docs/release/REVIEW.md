...
| P2 | README and project status still described rejected early art or pending reviews. | Root README and current state now identify approved v1; early project/verification documents are explicitly historical. |
...
| P2 | A local Git repository and same-disk ZIP are not off-site backup. | Still open: publish/copy the complete preservation set, including LFS data, to independently backed-up storage. |
...
The two largest residual risks are loss of the only local storage and future ability to run the archived macOS application. Keep redundant independent copies; retain original hardware/OS or a compatible environment when practical. A macOS app ZIP is not a VM image and does not include Apple drivers.

## Project boundary

This release completes the illustration. The historical web interaction experiment is not a working adventure game, SCI patch or verified Space Quest integration. Production-engine integration, walking/occlusion/inventory/save systems and any licensing decisions are not covered by v1 art approval.
