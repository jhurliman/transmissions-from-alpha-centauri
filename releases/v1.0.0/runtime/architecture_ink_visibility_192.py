...
Source ownership is captured in plate-runoff-192/ink-diagnostic/native-upper-strokes.json.
No screen-space trimming, geometry mutation or global line-width change.
"""
import bpy, math
PAIRS={
 '145 Panel 03 impact folded bottom':('Building side return.002',),
 'Panel field backing.003':('Building side return.002',),
 'Panel field backing.005':('Building side return.002',),
}
def install(scene,audit_path=None):
 ...
 assert all(vv and tt for vv,tt in data.values()),'192 side-return receiver missing'
 ...
 callback._guard192=True;callback._guard192_audit=audit
 parameter_editor.callbacks_modifiers_post[:]=[f for f in parameter_editor.callbacks_modifiers_post if not getattr(f,'_guard192',False)];parameter_editor.callbacks_modifiers_post.append(callback);flush();return audit

def apply(scene,embed=True,audit_path=None):
 def render_pre(scene,*args):install(scene,audit_path)
 render_pre._guard192=True;bpy.app.handlers.render_pre[:]=[f for f in bpy.app.handlers.render_pre if not getattr(f,'_guard192',False)];bpy.app.handlers.render_pre.append(render_pre);audit=install(scene,audit_path)
 if embed:
  from pathlib import Path
  name='192 Proven side-return occlusion guard.py';t=bpy.data.texts.get(name)or bpy.data.texts.new(name);t.clear();t.write(Path(__file__).read_text()+'\napply(bpy.context.scene,embed=False)\n');t.use_module=True
 return audit
