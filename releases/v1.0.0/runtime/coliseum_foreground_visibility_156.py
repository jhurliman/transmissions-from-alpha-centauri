...
import bpy
PAIRS={
 'Deck cross joist front flange.039':('Shoulder underside.002',),
 'Deck cross joist front flange.030':('Fascia bottom return.004',),
 'Long platform girder front flange.009':('Fascia bottom return.004',),
 'Folded fascia.004':('Fascia bottom return.004',),
 '091 bank slab.036':('Ground plinth.002',),
}
def install(scene):
 ...
 callback._guard156=True;callback._guard156_audit=audit
 parameter_editor.callbacks_modifiers_post[:]=[f for f in parameter_editor.callbacks_modifiers_post if not getattr(f,'_guard156',False)];parameter_editor.callbacks_modifiers_post.append(callback);return audit
def apply(scene,embed=True):
 def render_pre(scene,*args):install(scene)
 render_pre._guard156=True;bpy.app.handlers.render_pre[:]=[f for f in bpy.app.handlers.render_pre if not getattr(f,'_guard156',False)];bpy.app.handlers.render_pre.append(render_pre);audit=install(scene)
 if embed:
  from pathlib import Path
  name='156 Proven foreground occlusion guard.py';t=bpy.data.texts.get(name)or bpy.data.texts.new(name);t.clear();t.write(Path(__file__).read_text()+'\napply(bpy.context.scene,embed=False)\n');t.use_module=True
 return audit
