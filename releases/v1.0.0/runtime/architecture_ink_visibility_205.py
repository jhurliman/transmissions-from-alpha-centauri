...
Source ownership/depth is recorded in corner-ink-205/ownership.json and native-strokes.json.
No screen-space trimming, geometry mutation or global line-width change.
"""
import bpy, math
PAIRS={
 '145 Panel 01 top loss':('Building side return.002',),
 '145 Panel 01 top loss folded bottom':('Building side return.002',),
 '198 Architecture | layout_access.008 Folded sheet face.034':('Building side return.002',),
}
def install(scene,audit_path=None):
 ...
 assert all(vv and tt for vv,tt in data.values()),'205 side-return receiver missing'
 ...
 callback._guard205=True;callback._guard205_audit=audit
 parameter_editor.callbacks_modifiers_post[:]=[f for f in parameter_editor.callbacks_modifiers_post if not getattr(f,'_guard205',False)];parameter_editor.callbacks_modifiers_post.append(callback);flush();return audit

def apply(scene,embed=True,audit_path=None):
 def render_pre(scene,*args):install(scene,audit_path)
 render_pre._guard205=True;bpy.app.handlers.render_pre[:]=[f for f in bpy.app.handlers.render_pre if not getattr(f,'_guard205',False)];bpy.app.handlers.render_pre.append(render_pre);audit=install(scene,audit_path)
 if embed:
  from pathlib import Path
  name='205 Proven lower-return occlusion guard.py';t=bpy.data.texts.get(name)or bpy.data.texts.new(name);t.clear();t.write(Path(__file__).read_text()+'\napply(bpy.context.scene,embed=False)\n');t.use_module=True
 return audit
