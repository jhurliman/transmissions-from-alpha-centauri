...
KICK_RISE=.52
WINDOW_SETBACK=KICK_RISE*math.tan(math.radians(15))
KICK_WIDTH=1.98 # Exact clear width between the .40 m side frame faces.
def sloped_kick():
 # Front surface shares Y=0 with outer facade; top shares slider-frame setback.
 w=KICK_WIDTH/2;h=KICK_RISE;d=WINDOW_SETBACK;t=.024
 vs=[(-w,0,0),(w,0,0),(w,d,h),(-w,d,h),(-w,t,0),(w,t,0),(w,d+t,h),(-w,d+t,h)]
 mesh('Continuous frame-to-frame kick',vs,[(0,1,2,3),(7,6,5,4),(0,4,5,1),(1,5,6,2),(2,6,7,3),(3,7,4,0)],paint,.002)
 # Lower folded edge stays behind the common facade plane.
 box('Kick lower folded return',(0,.023,-.014),(KICK_WIDTH,.046,.032),steel)
register('kick_sloped_15',sloped_kick,{'kind':'kick_panel','width':KICK_WIDTH,'vertical_rise':KICK_RISE,'rake_from_vertical':15,'top_y':WINDOW_SETBACK,'bottom_y':0,'outward':[0,-1,0],'top_z':KICK_RISE,'bottom_z':0,'side_seam':0,'top_mates':'window perimeter and front slider frame'})
...
for state in ['closed','open','broken']:
 register('window_slider_'+state,lambda state=state:window(state),{'kind':'opening','rough_width':1.94,'rough_height':2.04,'depth':.88,'outward':[0,-1,0],'up':[0,0,1],'sash_count':2,'track_depths':[0,.075],'sash_travel':.82,'state':state,'no_transom':True,'broken_glass_is_variant':state=='broken'})
...
def window_bay(state='closed'):
 ...
 for z,h in [(.25,.5),(2.855,.49)]:box('Header / sill backing',(0,.42 if z<1 else .24,z),(1.94,.48,h),dark)
 for x in [-1.19,1.19]:
  for z in [.02,1.464]:inst('plate_narrow',(x,0,z))
 inst('kick_sloped_15',(0,0,.02));inst('plate_header',(0,0,2.60));inst('window_slider_'+state,(0,WINDOW_SETBACK,.54))
 # Side and upper reveals connect the outer facade to the inset perimeter.
 for x in [-.982,.982]:box('Inset window jamb return',(x,WINDOW_SETBACK/2,1.56),(.016,WINDOW_SETBACK,2.04),paint)
 box('Inset window head return',(0,WINDOW_SETBACK/2,2.58),(1.98,WINDOW_SETBACK,.025),paint)
 for x in [-1.19,1.19]:
