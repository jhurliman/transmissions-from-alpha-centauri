...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-116';O.mkdir(parents=True,exist_ok=True)
...
from coliseum_detail_116 import refine
bpy.ops.wm.open_mainfile(filepath=str(O/'rotation/plus4/scene.blend'));s=bpy.context.scene;C=bpy.data.collections['110 Coliseum detailed front ruin']
(O/'material-audit.json').write_text(json.dumps(refine(C),indent=2));s.render.use_freestyle=True
