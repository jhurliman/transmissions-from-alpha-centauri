...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-188';a=Image.open(R/'art/studies/coliseum-173/main-4k.png').convert('RGB');b=Image.open(O/'main-4k.png').convert('RGB');aa=np.array(a).astype('int16');bb=np.array(b).astype('int16');assert aa.shape==bb.shape==(2885,3840,3)
...
r={'baseline':'173main-4k.png','native_size':[3840,2885],'landmark_region':[1250,300,2500,1300],'outside_landmark_pixels_over4':int(((d>4)&outside).sum()),'outside_landmark_max_channel_delta':int(d[outside].max()),'outside_regions':rows,'note':'Inspect all meaningful outside regions; geometry/data preservation does not imply identical Freestyle raster.'}
core_outside=np.ones(d.shape,bool)
for x0,y0,x1,y1 in [(2080,545,2310,775)]:core_outside[y0:y1,x0:x1]=False
r['outside_right_break_region_pixels_over4']=int(((d>4)&core_outside).sum())
r['outside_right_break_region_max_channel_delta']=int(d[core_outside].max())
for name,box in [('right_break',(2080,545,2310,775)),('primary',(1450,430,1650,640)),('secondary',(2065,540,2300,790))]:
