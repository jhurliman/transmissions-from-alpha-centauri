...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/reviews/xenon-066';O.mkdir(exist_ok=True)
proofs=json.loads((R/'art/reviews/xenon-061/validation.json').read_text())
for label,maximum in [('A',.75),('B',.90),('C',1.00)]:
 ...
 for hostName,a in zip(['Architecture | buttress_45','Front-left section instance'],proofs):
  ...
  ramp=nt.nodes.new('ShaderNodeMapRange');ramp.label='066 Depth darkness | '+label;ramp.clamp=True;nt.links.new(dot.outputs['Value'],ramp.inputs['Value']);ramp.inputs['From Min'].default_value=0;ramp.inputs['From Max'].default_value=depth;ramp.inputs['To Min'].default_value=.9;ramp.inputs['To Max'].default_value=1-maximum
  ...
  records.append({'host':hostName,'depth_max_m':depth,'opening_darkening':.1,'maximum_darkening':maximum,'space':'linear scene color','reflection':'unchanged'})
