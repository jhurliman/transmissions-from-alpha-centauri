...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/reviews/xenon-063'
...
for hostName,a in zip(['Architecture | buttress_45','Front-left section instance'],proofs):
 ...
 m=bpy.data.materials.new('063 Mineral fracture | '+hostName);m.use_nodes=True;nt=m.node_tree;nt.nodes.clear();geo=nt.nodes.new('ShaderNodeNewGeometry');sub=nt.nodes.new('ShaderNodeVectorMath');sub.operation='SUBTRACT';nt.links.new(geo.outputs['Position'],sub.inputs[0]);sub.inputs[1].default_value=a['center'];dot=nt.nodes.new('ShaderNodeVectorMath');dot.operation='DOT_PRODUCT';nt.links.new(sub.outputs[0],dot.inputs[0]);dot.inputs[1].default_value=tuple(-v for v in a['normal'])
 ramp=nt.nodes.new('ShaderNodeMapRange');ramp.clamp=True;nt.links.new(dot.outputs['Value'],ramp.inputs['Value']);ramp.inputs['From Min'].default_value= .0004;ramp.inputs['From Max'].default_value= .004;ramp.inputs['To Min'].default_value=1;ramp.inputs['To Max'].default_value=0
 bs=nt.nodes.new('ShaderNodeBsdfPrincipled');bs.inputs['Base Color'].default_value=(.0585,.0533,.0559,1);bs.inputs['Metallic'].default_value=0;bs.inputs['IOR'].default_value=1.48
 ...
 records.append({'host':hostName,'mouth_specular_fade_m':[.0004,.004],'interior_roughness':.93,'mouth_roughness':.28,'ior':1.48})
