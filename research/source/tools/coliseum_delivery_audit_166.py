"""Read-only fresh166scene/kit geometry, normals, materials and dependency audit."""
import bpy,json,hashlib,array
from pathlib import Path
R=Path.cwd();O=R/'art/studies/coliseum-166'
def ids():
 rows=[]
...
A=read('art/studies/coliseum-166/scene.blend');B=read('art/studies/coliseum-166/kit.blend');out={}
for key,d in [('scene166',A),('kit166',B)]:out[key]={k:v for k,v in d.items()if k!='stamp'}
a=A['stamp'];b=B['stamp'];out['kit166_vs_scene166']={'expected_objects':len(a),'actual_objects':len(b),'missing':sorted(set(a)-set(b)),'extra':sorted(set(b)-set(a)),'changed':{k:[f for f,vv in v.items()if vv!=b.get(k,{}).get(f)]for k,v in a.items()if v!=b.get(k)}}
assert not out['kit166_vs_scene166']['missing'] and not out['kit166_vs_scene166']['extra'] and not out['kit166_vs_scene166']['changed']
