...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-126/fracture-seed-v2';sys.path.insert(0,str(R/'tools'))
 ...
 for ni,name in enumerate(NAMES):
  ...
  for ci,c in enumerate(centers):
   for k in range(rng.choice([2,3,4])):
    center=c+rng.uniform(-.27,.27);width=rng.uniform(.20,.48);depth=rng.uniform(.36,.75)
  ...
  for top in [False,True]:
   for rr in rs:
    for u in us:
     z=62.559986
     if top:
      base=piece(u,ctrl);front=max((piece(u,c)for c in notches),default=0);frontweight=max(0,1-(75-rr)/1.55);rearweight=max(0,1-(rr-67)/1.4)*.9
