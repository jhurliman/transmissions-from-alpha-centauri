...
def run(C):
 ...
 for f in bm.faces:
  ...
  c=[cc[v.index]for v in f.verts];protected=(all(p[0]>=74.85 for p in c)or all(p[0]<=67.15 for p in c)or all(p[1]<amin+.0003 and p[2]<66.1 for p in c)or all(p[1]>amax-.0003 and p[2]<66.1 for p in c)or all(p[2]<floor+.05 for p in c))and f.calc_area()>=1e-10
 ...
 bpy.data.libraries.write(str(R/'art/studies/coliseum-160/right-repair/candidate.blend'),{collection},fake_user=True)
 return d
if __name__=='__main__':
 bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-156/scene.blend'));d=run(bpy.data.collections['110 Coliseum detailed front ruin']);(R/'art/studies/coliseum-160/right-repair/strip-candidate-audit.json').write_text(json.dumps(d,indent=2));print(json.dumps(d),flush=True)
