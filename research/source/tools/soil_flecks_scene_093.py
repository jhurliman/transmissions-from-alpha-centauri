...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/soil-093';bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/soil-092/scene-B.blend'));s=bpy.context.scene;s.render.threads_mode='FIXED';s.render.threads=4
g=s.objects['Street foundation'];targets=[n for n in g.data.materials[0].node_tree.nodes if n.type=='TEX_IMAGE' and n.image and 'pigment-B' in n.image.name];assert len(targets)==1
for label in ['A','B','C']:
