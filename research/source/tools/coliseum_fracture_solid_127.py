...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-127/fracture-solid';sys.path.insert(0,str(R/'tools'))
...
def apply(C):
 ...
 rm=ob.modifiers.new('127 final closed solid','REMESH');rm.mode='VOXEL';rm.voxel_size=.04;rm.adaptivity=0.;rm.use_smooth_shade=False;bpy.ops.object.modifier_apply(modifier=rm.name);check=topology(ob)
 ...
 return {'references':['UCL-01','UCL-02','DP-03'],'method':'native bounded convex negative cuts on validated voxel foundation; no displaced surfaces','cuts':rows,'outside_reconstruction_max_m':outside,'voxel_tolerance':.04,'outside_numeric_guard_passed':outside<=.08,'final':topology(ob),'source_foundation':'126 volumetric; documented thin-surface removal remains pending visual review','production_integrated':False}
