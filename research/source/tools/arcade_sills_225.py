...
def clip_lost_contacts(scene,oldtree,oldvs,C):
 ...
 for rec in old:
  for si,st in enumerate(rec['strokes']):
   ...
   def unsupported(t):
    ...
    return all(a<=p[k]<=b for k,(a,b)in enumerate(box)) and oldtree.find_nearest(p)[3]<.012
   ts=[0.]
 ...
 # Certify former-owner proximity continuously. The removed platform contact
 # is obsolete even where its OTHER owner (a jamb or tunnel wall) remains.
 leaves=[]
 for row in changes:
  st=next(r for r in old if r['layer']==row['layer']and r['frame']==row['frame'])['strokes'][row['stroke']]
  def p_at(t):return M@Vector(interpolate(st,t)['position'])
  def verify(a,b,depth=0):
   pa,pb,pm=p_at(a),p_at(b),p_at((a+b)/2);upper=oldtree.find_nearest(pm)[3]+(pb-pa).length/2+.0001
   if upper<.02:leaves.append(upper);return
   if depth>=30:raise RuntimeError('225 source-owner proximity certificate failed')
   mid=(a+b)/2;verify(a,mid,depth+1);verify(mid,b,depth+1)
  for a,b in row['removed_parameter_intervals']:
   ts=[a]+[float(i)for i in range(math.floor(a)+1,math.ceil(b))if a<i<b]+[b]
   for x,y in zip(ts,ts[1:]):verify(x,y)
 cert={'method':'Continuous1-Lipschitz distance upper bound to removed sill/platform face; the contact loses this owner even if another supporting wall remains','source_owner_tolerance_m':.02,'sample_selection_m':.012,'numerical_margin_m':.0001,'removed_intervals':sum(len(r['removed_parameter_intervals'])for r in changes),'leaf_count':len(leaves),'maximum_certified_source_distance_m':max(leaves),'all_intervals_certified':True}
 ...
 return {'changed_strokes':len(changes),'removed_intervals':cert['removed_intervals'],'continuous_removed_owner_certificate':cert,'unaffected_attributes_exact':True,'before_digest':digest(old),'after_digest':digest(after),'changes':[{k:v for k,v in r.items()if k!='runs'}for r in changes]}
