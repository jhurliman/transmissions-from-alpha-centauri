...
def apply(scene):
 ...
 for name,origin,radii,normal,group in SITES:
  ...
  for sl in ob.material_slots:
   ...
   strengths={'127 Broken sponge pigment at 4K scale':.85}
   for q in list(n):
   ...
   # Attenuate the final dark pigment mix, after every nonlinear threshold.
   # Noise still drives unmodified light flecks, specular catches and all AO.
   def linear_hex(h):
    a=[int(h[i:i+2],16)/255 for i in (0,2,4)]
    return tuple(v/12.92 if v<=.04045 else ((v+.055)/1.055)**2.4 for v in a)
   targets={'100e23':('Grime branches',.90),'201a30':('Interrupted vertical runoff',.88),'242039':('Connected worn pigment islands',.85),'39323e':('Fine stone pits and dry pigment',.90)}
   found=[]
   for q in list(n):
    if q.type!='MIX_RGB' or q.inputs[2].is_linked or not q.inputs[0].is_linked:continue
    for hx,(label,strength) in targets.items():
     if max(abs(q.inputs[2].default_value[i]-linear_hex(hx)[i]) for i in range(3))>1e-5:continue
     original=q.inputs[0].links[0].from_socket;todo=[original.node];seen=set();labels=[];types=[]
     while todo:
      v=todo.pop()
      if v.name in seen:continue
      seen.add(v.name);labels.append(v.label);types.append(v.type);todo.extend(k.from_node for inp in v.inputs for k in inp.links)
     if label not in labels:continue
     if 'AMBIENT_OCCLUSION' in types:raise RuntimeError('Refuse to attenuate AO-driven mix '+q.name)
     factor=calc('MULTIPLY',original,calc('SUBTRACT',1,calc('MULTIPLY',gate,strength)));l.new(factor,q.inputs[0]);found.append(label)
     changes.append({'node':q.name,'label':label,'dark_color_hex':hx,'post_threshold_factor_reduction':strength,'source_factor_node':original.node.name,'factor_has_AO':False})
   if 'Grime branches' not in found:raise RuntimeError('Missing causal grime dark mix '+name)
