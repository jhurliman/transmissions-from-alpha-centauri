...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/corner-ink-205';O.mkdir(parents=True,exist_ok=True);bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/scene-completion-203/scene.blend'));s=bpy.context.scene
for ob in s.objects:
...
for inst in dg.object_instances:
 ...
 ob.to_mesh_clear()
report=dict(source='203',roi=roi,ray_summary=dict(collections.Counter(r['object']for r in rows)),rays=rows,candidate_objects=bounds,edges=edges)
