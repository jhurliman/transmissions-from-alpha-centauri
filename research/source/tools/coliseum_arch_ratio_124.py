...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-124/arch-ratio';sys.path.insert(0,str(R/'tools'))
...
def apply(C,reduction):
 if reduction not in [.05,.10,.20]:raise ValueError('Expected .05, .10, or .20')
 original,world,unpack=mapping();scale=1-reduction;half=75*math.tau/36*.34;rows=[];tunnels={};parts=[]
 ...
 for (tier,bay),ob in tunnels.items():
  ac=-math.pi+(bay+.5)*math.tau/36;bottom=2.73+tier*18.33;base=bottom+.35;top=bottom+18.33;spring=top-2.184-half;crown=spring+half;nspring=base+scale*(spring-base);ncrown=base+scale*(crown-base);old=ob.data;before=topology(ob);iv=ob.matrix_world.inverted();coords=[unpack(ob.matrix_world@v.co)for v in old.vertices];uv=[(a-ac)*75 for r,a,z in coords];sidefaces=[]
  ...
  for (rr,a,z),u in zip(coords,uv):
   if z<=crown+.0001:zz=base+scale*(z-base);uu=u*scale
   else:
    t=(z-crown)/(top-crown);zz=ncrown+t*(top-ncrown);uu=u*(scale+(1-scale)*t)
  ...
  for sign in [-1,1]:
   ...
   for front in [False,True]:
    low=ids[(front,False)];high=ids[(front,True)];rr=coords[low][0];outeru=uv[high];inneru=uv[low]*scale
  ...
  successful.add((tier,bay));ob['124 arch reduction']=reduction;rows.append({'object':ob.name,'before':before,'after':after,'clear_width_authored_before':half*2,'clear_width_authored_after':half*2*scale,'clear_height_authored_before':crown-base,'clear_height_authored_after':(crown-base)*scale,'base_unchanged':base,'floor_top_unchanged':top,'new_side_masonry_joined':True})
 moved=[]
 for ob in parts:
  tier,bay=int(ob['tier']),int(ob['bay'])
  if (tier,bay)not in successful:continue
  ac=-math.pi+(bay+.5)*math.tau/36;base=2.73+tier*18.33+.35;top=2.73+tier*18.33+18.33;spring=top-2.184-half;nspring=base+scale*(spring-base);iv=ob.matrix_world.inverted();ob.data=ob.data.copy()
  ...
  for v in ob.data.vertices:
   ...
   if 'archivolt'in ob.name:
    r=math.hypot(u,z-spring);theta=math.atan2(z-spring,u);newr=r-half*(1-scale);uu=newr*math.cos(theta);zz=nspring+newr*math.sin(theta)
   elif 'jamb'in ob.name:uu=u-math.copysign(half*(1-scale),u);zz=base+scale*(z-base)
   else:uu=u-math.copysign(half*(1-scale),u);zz=z+(nspring-spring)
   v.co=iv@world(rr,ac+uu/75,zz);attr.data[v.index].vector=original(rr,ac+uu/75,zz)
  ob.data.update();ob['124 arch reduction']=reduction;moved.append(ob.name)
 return {'references':['UCL-01','UCL-03','DP-03'],'reduction':reduction,'true_opening_scale':scale,'preserved':['floor bands','bay exterior widths','column footprints','radial wall depth','front-quarter arch-light parameter'],'wall_changes':rows,'molding_jamb_impost_changes':moved,'successful_bays':len(successful),'column_agent_independent':True}

if __name__=='__main__':
 import os
 reduction=float(os.environ.get('ARCH_REDUCTION','.10'));key=str(round(reduction*100)).zfill(2);out=O/key;out.mkdir(parents=True,exist_ok=True);bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-123/scene.blend'));audit=apply(bpy.data.collections['110 Coliseum detailed front ruin'],reduction);(out/'audit.json').write_text(json.dumps(audit,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(out/'geometry.blend'));print('ARCH_RATIO',key,audit['successful_bays'],flush=True)
