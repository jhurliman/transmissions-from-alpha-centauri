...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-139';O.mkdir(parents=True,exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/scene-details-138/scene.blend'));s=bpy.context.scene;C=next(c for c in s.collection.children_recursive if c.name=='110 Coliseum detailed front ruin' and c.library is None)
...
report={'source':'scene-details-138','seconds':time.time()-t,'mesh_objects':len(rows),'unique_meshes':len(cache),'issue_objects':len(issues),'issues':issues,'all':rows,'limitations':['Raw native meshes before modifiers; evaluated geometry must be checked on revised specimens.','Open flat detail meshes may be intentional; issue list is diagnostic, not automatic failure.','Does not establish all inter-object contact or absence of floating fragments.','No triangle self-crossing test in this broad inventory; see separate135evaluated visibility audit for known remaining folded meshes.']}
(O/'geometry-health-current.json').write_text(json.dumps(report,indent=2));print(json.dumps({k:report[k] for k in ['seconds','mesh_objects','unique_meshes','issue_objects']}),flush=True)
