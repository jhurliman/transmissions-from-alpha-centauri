     ...
     if q.type=='MATH' and q.operation=='MULTIPLY':
      for socket in q.inputs:
       if not socket.is_linked:
        ...
        elif abs(socket.default_value-.08)<.001:
         socket.default_value=.08+.10*strength
         for srcinp in q.inputs:
          if srcinp.is_linked:
           upstream=srcinp.links[0].from_socket;norm=n.new('ShaderNodeMath');norm.operation='MULTIPLY';norm.inputs[1].default_value=.45;norm.label='Normalize actual diffuse into painted range';l.new(upstream,norm.inputs[0]);l.new(norm.outputs[0],srcinp)
   ...
   if spec:
    ...
    for _ in range(12):
     ...
     for node in frontier:
      ...
      seen.add(node)
      if node.type=='MATH' and node.operation=='MULTIPLY' and any(not z.is_linked and abs(z.default_value-.21)<.0001 for z in node.inputs):
       for inp in node.inputs:
        if not inp.is_linked and abs(inp.default_value-.21)<.0001:inp.default_value=.21+.09*strength
       g=next(v for v in n if v.type=='NEW_GEOMETRY');sep=n.new('ShaderNodeSeparateXYZ');l.new(g.outputs['Normal'],sep.inputs[0]);gate=n.new('ShaderNodeMapRange');gate.clamp=True;gate.inputs['From Min'].default_value=.15;gate.inputs['From Max'].default_value=.65;l.new(sep.outputs['Z'],gate.inputs['Value']);oldlinks=list(node.outputs[0].links);masked=n.new('ShaderNodeMath');masked.operation='MULTIPLY';l.new(node.outputs[0],masked.inputs[0]);l.new(gate.outputs[0],masked.inputs[1]);
       for oldlink in oldlinks:l.new(masked.outputs[0],oldlink.to_socket)
    ...
    if node.type=='MATH' and node.operation=='MULTIPLY':
     for inp in node.inputs:
      if not inp.is_linked:
       if abs(inp.default_value-.38)<.0001:inp.default_value=.32
       elif abs(inp.default_value-.90)<.0001:inp.default_value=1.0
