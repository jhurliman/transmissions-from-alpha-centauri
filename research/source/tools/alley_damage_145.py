...
def apply(scene=None):
 ...
 paint=make_panel_material();core=mat('145 Exposed substrate',(0.24,.175,.12));dark=mat('145 Deep unlit backing steel',(.024,.031,.041),.91);slit=mat('145 Dark fracture interior',(.009,.006,.012),.96)
 panels=[];footprints={}
 for i,variant in enumerate(['top loss','facing pits','impact','crack']):
  x=i*(W+.022);o=box('145 Panel %02d %s'%(i+1,variant),(x,0,0),(x+W,T,H),C);o.data.materials.append(paint);o.data.materials.append(core);o.data.materials.append(slit);o['145 variant']=variant
  footprints[o.name]=[]
  if i==0:
  ...
  if i==2:
   ...
   rings=[[(x+cx+(u-cx)*(1.20+.20*math.sin(k*1.7)),-.016,cz+(z-cz)*(1.16+.13*math.cos(k*2.1)))for k,(u,z) in enumerate(p)],[(x+u,.004,z)for u,z in p],[(x+cx+.024+(u-cx)*.64,.05,cz+.012+(z-cz)*.79)for u,z in p]];cut(o,cutter('Irregular through impact entry',rings,C));footprints[o.name].append(p)
  if i==3:
   p=[(.29,1.43),(.303,1.27),(.337,1.232),(.431,1.137),(.412,1.058),(.399,1.00),(.444,.934),(.474,.861),(.483,.781),(.472,.735)]
   widths=[.0060,.0072,.0050,.0038,.0052,.0057,.0044,.0030,.0020,.0007]
   # One finite tapered polygon: depth stays physical and width responds to the fracture.
   left=[];right=[]
   for k,(a,b) in enumerate(p):
    prev=Vector(p[max(0,k-1)]);nxt=Vector(p[min(len(p)-1,k+1)]);e=(nxt-prev).normalized();q=Vector((-e.y,e.x))*widths[k]
    left.append((a+q.x,b+q.y));right.append((a-q.x,b-q.y))
   outline=left+list(reversed(right));rings=[[(x+u,-.01,z)for u,z in outline],[(x+u,.009,z)for u,z in outline]];cut(o,cutter('Finite varying-width dark crack',rings,C))
   footprints[o.name].append(p)
  # Assign true cut returns/floors only, keep intact broad enamel faces unchanged.
  for p in o.data.polygons:
   if any(T-.00001>o.data.vertices[v].co.y>1e-5 for v in p.vertices) and not all(abs(o.data.vertices[v].co.y-T)<1e-5 for v in p.vertices):p.material_index=2 if i==3 else 1
