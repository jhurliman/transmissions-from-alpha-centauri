...
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/soil-tracks-246/scene.blend'));s=bpy.context.scene
...
for name,amount in [('shortened',1.5)]:
 bpy.data.objects['Street foundation'].material_slots[0].material=orig;a=apply(s);s.render.filepath=str(O/(name+'.png'));bpy.ops.render.render(write_still=True)
