...
def apply(scene):
 ...
 for j,o in enumerate(sorted(lips,key=lambda o:o.name)):
  ...
  for k in range(6 if j==0 else 9):
   ...
   events.append({'vent':o.name,'origin':[x,y,za],'impact':list(hit),'fall_m':dist,'downslope':list(down),'width':rng.uniform(.055,.14),'strength':rng.uniform(.28,.90),'drift':rng.uniform(-.035,.035)})
 private=bpy.data.collections.new('251 Private vent runoff awning');private.use_fake_user=True;private.instance_offset=src.instance_offset
 changes=[]
 for o in src.objects:
  ...
  for sl in q.material_slots:
   base=sl.material;m=base.copy();m.name='251 Vent runoff | '+base.name;p=Paint(m);op=p.op
