...
def apply(scene):
 ...
 events=[];rng=random.Random(250)
 for j,o in enumerate(sorted(lips,key=lambda o:o.name)):
  pts=[root.matrix_world@o.matrix_world@Vector(v)for v in o.bound_box]
  x=min(v.x for v in pts);za=min(v.z for v in pts);lo=min(v.y for v in pts)+.10;hi=max(v.y for v in pts)-.10
  for k in range(4 if j==0 else 6):
   y=lo+(hi-lo)*(k+rng.uniform(.25,.75))/(4 if j==0 else 6)
   hit,normal,_,dist=bvh.ray_cast(Vector((x,y,za)),Vector((0,0,-1)))
   assert hit is not None
   g=Vector((0,0,-1));down=(g-normal*g.dot(normal)).normalized()
   events.append({'vent':o.name,'origin':[x,y,za],'impact':list(hit),'fall_m':dist,'downslope':list(down),'width':rng.uniform(.07,.18),'strength':rng.uniform(.54,.83),'drift':rng.uniform(-.035,.035)})
 private=bpy.data.collections.new('250 Private vent runoff awning');private.use_fake_user=True;private.instance_offset=src.instance_offset
 changes=[]
 for o in src.objects:
  ...
  for sl in q.material_slots:
   base=sl.material;m=base.copy();m.name='250 Vent runoff | '+base.name;p=Paint(m);op=p.op
   ...
   for e in events:
    ...
    trail=op('MULTIPLY',trail,p.remap(coarse,.20,.72,.45,1))
   ...
   dark=p.mix(1,old,(.32,.34,.40,1),'Cool soot-bearing runoff','MULTIPLY')
 ...
 return {'vents':len(lips),'drips':events,'targets':changes,'method':'Vertical ray from outer lower vent sill into actual roof mesh; tangent gravity sets downslope. Narrow irregular material trails from impact to eave. No fluid dynamics claim.','geometry_unchanged':True,'haze_unchanged':True}
