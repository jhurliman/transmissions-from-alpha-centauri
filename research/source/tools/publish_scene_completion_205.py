...
import json,html
R=Path(__file__).resolve().parents[1];O=R/'art/studies/scene-completion-205';B=R/'art/studies/beam-rust-197'
frames={
...
for key,title,desc in sections:
 ...
 if key=='transition':extra='<details><summary>Palette source · unchanged foreground junk pile</summary><img src="/art/studies/scene-completion-205/after-palette-source.png"><p>The middle area retains half its original color and receives half of this lighting-driven blue/rust palette.</p></details>'
 ...
 parts.append(f'<section id="{key}"><h2>{title}</h2><p>{desc}</p><div class="pair"><figure><img loading="lazy" src="/art/studies/scene-completion-205/before-{key}.png"><figcaption>Before · 197</figcaption></figure><figure><img loading="lazy" src="/art/studies/scene-completion-205/after-{key}.png"><figcaption>Combined update · 205</figcaption></figure></div>{extra}</section>')
nav=' · '.join(f'<a href="#{k}">{t}</a>'for k,t,_ in sections)
p='''<!doctype html><html><meta charset="utf-8"><title>205 · Combined scene weathering</title><style>body{max-width:1800px;margin:24px auto;padding:0 20px;background:#211f26;color:#eee6de;font:16px/1.5 system-ui}a{color:#efb38f}img{max-width:100%;display:block}nav{position:sticky;top:0;background:#211f26ed;padding:12px;z-index:1}section{border-top:1px solid #554b50;padding:25px 0;scroll-margin-top:100px}.pair{display:flex;gap:22px;align-items:flex-start}.pair figure{flex:1;min-width:0;margin:0}.pair img{width:100%;max-height:1200px;object-fit:contain;object-position:top}figcaption{padding:8px}button{background:#5c4945;color:#fff;border:1px solid #ad8274;padding:10px 18px;margin:10px 10px 10px 0;border-radius:4px;cursor:pointer}details{margin:20px 0}details img{max-height:900px}#main{width:100%}</style><h1>205 · Combined scene weathering</h1><p>All seven work areas brought into one current scene. Before/after views below use the same camera. Final artistic approval remains with you.</p><nav>'''+nav+'''</nav><button onclick="pick('after')">Combined update</button><button onclick="pick('before')">Before</button><img id="main" src="/art/studies/scene-completion-205/after-display.png"><p><a href="/art/studies/scene-completion-205/main-4k.png">Full-resolution render</a> · <a href="/art/studies/scene-completion-205/scene.blend">Editable scene</a></p>'''+''.join(parts)+'''<script>function pick(v){document.getElementById('main').src='/art/studies/scene-completion-205/'+v+'-display.png'}</script></html>'''
(R/'prototype/review-205.html').write_text(p)
