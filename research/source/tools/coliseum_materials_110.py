...
def masonry(role='wall'):
 m=bpy.data.materials.new('110 Painted masonry '+role);m.use_nodes=True;n=m.node_tree.nodes;l=m.node_tree.links;n.clear()
 def node(t,label=''):
 ...
 colors=[(0,'242344'),(.25,'463751'),(.43,'65505c'),(.65,'92715e'),(.86,'ae896f'),(1,'bc987c')]
 ...
 orient=node('ShaderNodeVectorMath','Broad form lighting');orient.operation='DOT_PRODUCT';l.new(g.outputs['Normal'],orient.inputs[0]);orient.inputs[1].default_value=(.38,-.67,.64);light=math('ADD',math('MULTIPLY',orient.outputs['Value'],.82),math('MULTIPLY',bw.outputs[0],.18));l.new(light,pal.inputs[0]);color=pal.outputs[0]
 # Coherent broken pigment islands, restrained coverage with quiet flat interiors.
 pigment=noise(.38,'Connected worn pigment islands');mask=math('MULTIPLY',ramp(pigment,.60,.68),.60);color=mix(mask,color,rgba('242039'))
 # Fine pigment flecks/pits, subordinate to broad color regions.
 fine=noise(8.0,'Fine stone pits and dry pigment');pits=math('MULTIPLY',ramp(fine,.64,.75),.48);color=mix(pits,color,rgba('39323e'));fleck=math('MULTIPLY',math('SUBTRACT',1,ramp(fine,.24,.34)),.20);color=mix(fleck,color,rgba('b68b70'))
