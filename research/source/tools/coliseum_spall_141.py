"""141 V4: one broad U9 facing loss; no geometry outside the source perimeter."""
...
def apply(C):
 ...
 for name,index,depth,variant in TARGETS:
  ...
  ring=[(.05,.08),(.27,.07),(.29,.13),(.55,.10),(.76,.27),(.83,.55),(.66,.73),(.34,.63),(.12,.41),(.04,.26)]
  ring=[(u,1-v)for u,v in ring[::-1]]
  uv=boundary+[Vector(q)for q in ring+[(.38,.57),(.67,.42)]];edges=[(i,(i+1)%B)for i in range(B)]+[(B+i,B+(i+1)%len(ring))for i in range(len(ring))]
