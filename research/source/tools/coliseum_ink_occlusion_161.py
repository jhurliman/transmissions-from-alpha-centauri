...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-161/visibility';strokes=json.loads((O/'native-strokes.json').read_text());bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-159/scene.blend'));s=bpy.context.scene
for ob in s.objects:
...
bpy.context.view_layer.update();dg=bpy.context.evaluated_depsgraph_get();origin=s.camera.matrix_world.translation;boxes={'29':[3425,843,3481,885],'38':[616,909,660,949],'25':[3701,813,3746,852]};rows=[]
