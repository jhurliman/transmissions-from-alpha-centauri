...
def run(C):
 ...
 d={'source':'156','pre112_domain':'Radially extruded concave profile including side notch; pre112-domain.json','object':NAME,'protected':'Exact nonempty front/back/bottom/left-end faces; right angular end retained below authoredz66.1. Only cap and upper right folded end strip reconstructed.','before_crossings':len(robust_crossings(types.SimpleNamespace(data=src,matrix_world=M))),'accepted':False}
 ...
 for f in bm.faces:
  ...
  c=[cc[v.index]for v in f.verts];protected=(all(p[0]>=74.85 for p in c)or all(p[0]<=67.15 for p in c)or all(p[1]<amin+.0003 for p in c)or all(p[1]>amax-.0003 and p[2]<66.1 for p in c)or all(p[2]<floor+.05 for p in c))and f.calc_area()>=1e-10
 ...
 if d['boundary_branch_vertices']:d['reason']='Protected boundary still branches';return d
 loops=[]
 while edges:
 ...
 if len(loops)!=1:d['reason']='Protected boundary not one connected return strip';return d
 ...
 def cumulative(seq):
  ...
  return length
 fs=cumulative(F);bs2=cumulative(B)
 ...
 for j in range(1,20):
  s=j/20;p,a=interp(F,fs,s);q,b=interp(B,bs2,s)
  for t in [.2,.4,.6,.8]:uv.append(Vector((s,t)));worldvals.append(p.lerp(q,t));attrs.append(a.lerp(b,t))
 vv,ee,ff,vorig,_,_=geometry.delaunay_2d_cdt(uv,[(i,(i+1)%len(loop))for i in range(len(loop))],[list(range(len(loop)))],1,1e-9,True);d['profile_chain_lengths']=[len(F),len(B),len(E1),len(E0)];d['cdt_generated_unmapped_vertices']=sum(not ids for ids in vorig);d['cdt_merged_input_vertices']=sum(len(ids)>1 for ids in vorig)
 ...
 bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.verts.index_update();attrvalues={v.index:(orig[v[vi]]if v[vi]>=0 else newattrs[v])for v in bm.verts};out=bpy.data.meshes.new('160 ruled return candidate');bm.to_mesh(out);bm.free();out.update()
 ...
 candidate=bpy.data.objects.new('160 isolated U10L candidate',out);collection=bpy.data.collections.new('160 isolated return candidate');bpy.context.scene.collection.children.link(collection);collection.objects.link(candidate);candidate.matrix_world=M
 bpy.data.libraries.write(str(R/'art/studies/coliseum-160/repair/candidate.blend'),{collection},fake_user=True)
 return d
if __name__=='__main__':
 bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-156/scene.blend'));d=run(bpy.data.collections['110 Coliseum detailed front ruin']);(R/'art/studies/coliseum-160/repair/strip-candidate-audit.json').write_text(json.dumps(d,indent=2));print(json.dumps(d),flush=True)
