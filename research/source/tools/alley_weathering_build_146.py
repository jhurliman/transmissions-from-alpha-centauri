...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/alley-weathering-146/actual';O.mkdir(exist_ok=True)
...
from alley_surface_details_146 import apply_panels
...
material=make_panel_material('146 Actual facade light response',origin=tuple(origin),across=tuple(u),up=tuple(v),span=(umax-umin,vmax-vmin),panel_regions=specs)
