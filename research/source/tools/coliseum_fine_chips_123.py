...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-123/chips'
...
def apply(C):
 ...
 for name in NAMES[:1]:
  ...
  for index,(center,radius,tangent,outward)in enumerate(attempts):
   ...
   try:
    bpy.ops.object.modifier_apply(modifier=mod.name);state=topology(ob);newtree,_,_=tree(ob);low=[min(v[k]for v in coords)-.004 for k in range(3)];high=[max(v[k]for v in coords)+.004 for k in range(3)]
    ...
    if len(ob.data.vertices)==len(old.vertices):ob.data=old
     ...
     slots=[i for i,ma in enumerate(m.materials)if ma and any(x in ma.name.lower()for x in ['fracture','core','cavity'])];slot=slots[0]if slots else 0
     for f in m.polygons:
  ...
  ob['123 fine negative chips']=accepted
 ...
 return {'method':'convex negative cutouts in irregular clusters on existing crown/breach edges','references':['UCL-01','UCL-02','DP-03'],'targets':NAMES[:1],'deferred':{NAMES[1]:'No mutation; polygon base retained. Render triangulation has four pre-existing nonmanifold edges, requiring separate normalization study.'},'cuts':rows,'new_objects':0}

def run():
 O.mkdir(parents=True,exist_ok=True);bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-122/scene.blend'));C=bpy.data.collections['110 Coliseum detailed front ruin'];from coliseum_base_123 import apply as prepare_base;prepare_base(C)
