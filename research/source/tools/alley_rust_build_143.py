...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/alley-rust-143';O.mkdir(exist_ok=True)
...
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/alley-rust-140/scene.blend'));s=bpy.context.scene
...
from alley_rust_143 import apply
result=apply(s);after=fingerprint(s);changes={k:[f for f in v if v[f]!=after[k][f]]for k,v in before.items()if v!=after[k]};allowed=set(result['changed_slots']);assert set(before)==set(after);assert all(k in allowed and fs==['materials']for k,fs in changes.items());ma=material_snapshot();assert all(v==ma[k]for k,v in mats.items())
