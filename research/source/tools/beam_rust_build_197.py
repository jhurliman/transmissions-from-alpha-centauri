...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/beam-rust-197'
...
if 'render' in sys.argv:
 ...
 bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/beam-rust-196/scene.blend'));s=bpy.context.scene
 ...
 from beam_edge_edit_197 import apply
 result=apply(s);after=fingerprint(s);changes={k:[f for f in v if v[f]!=after[k][f]]for k,v in before.items()if v!=after[k]};assert len(changes)==1 and all(set(v)=={'materials'}for v in changes.values());new=material_snapshot();assert all(new[k]==v for k,v in old.items())
