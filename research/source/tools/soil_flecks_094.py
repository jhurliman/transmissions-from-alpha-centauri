"""Nested additional scattered pigment over093C; existing sponge and broad marks fixed."""
from pathlib import Path
import numpy as np,json
from PIL import Image,ImageDraw
R=Path(__file__).resolve().parents[1];O=R/'art/studies/soil-094';base=np.array(Image.open(R/'art/studies/soil-093/pigment-C.png').convert('RGB'));H,W=base.shape[:2];rng=np.random.default_rng(94011)
...
audit={}
for label,nflecks,nsponges in [('A',68000,0),('B',128000,0),('C',298000,0)]:
 rng=np.random.default_rng(94011);masks=[Image.new('L',(W,H),0) for _ in range(3)];draw=[ImageDraw.Draw(m) for m in masks]
 ...
 audit[label]={'marked_texel_fraction':float(coverage.mean()),'additional_flecks':nflecks,'total_flecks':nflecks+42000,'relative_to_093A':(nflecks+42000)/85000,'fixed_sponge_dabs':1400,'sponge_dabs':nsponges,'mean_rgb_delta':(out-base).mean(axis=(0,1)).tolist()}
