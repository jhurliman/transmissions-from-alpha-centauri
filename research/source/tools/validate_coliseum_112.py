...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-112'
...
r={'baseline':'110','candidate':'112','nonlandmark_objects_compared':len(x),'object_changes':[k for k in x if x[k]!=y.get(k)],'unexpected_objects':sorted(set(y)-set(x)),'materials_compared':len(xm),'material_graph_changes':[k for k in xm if xm[k]!=ym.get(k)]};(O/'preservation.json').write_text(json.dumps(r,indent=2));print(r)
