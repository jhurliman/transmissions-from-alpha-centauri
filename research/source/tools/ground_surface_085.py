...
def mineral(name,base):
 ...
 scale=n.new('ShaderNodeMapRange');scale.clamp=True;scale.inputs['From Min'].default_value=.2;scale.inputs['From Max'].default_value=1.5;l.new(bw.outputs[0],scale.inputs[0]);geo=n.new('ShaderNodeNewGeometry');dp=n.new('ShaderNodeVectorMath');dp.operation='DOT_PRODUCT';dp.inputs[1].default_value=(-.46,-.5,.735);l.new(geo.outputs['Normal'],dp.inputs[0]);local=n.new('ShaderNodeMapRange');local.clamp=True;local.inputs['From Min'].default_value=.10;local.inputs['From Max'].default_value=.95;l.new(dp.outputs['Value'],local.inputs[0]);l.new(local.outputs[0],r.inputs[0]);gl=n.new('ShaderNodeBsdfGlossy');gl.inputs['Roughness'].default_value=.36;gr=n.new('ShaderNodeShaderToRGB');l.new(gl.outputs[0],gr.inputs[0]);gb=n.new('ShaderNodeRGBToBW');l.new(gr.outputs[0],gb.inputs[0]);g=n.new('ShaderNodeMapRange');g.clamp=True;g.inputs['From Min'].default_value=.22;g.inputs['From Max'].default_value=1.8;g.inputs['To Max'].default_value=.2;l.new(gb.outputs[0],g.inputs[0]);mix=n.new('ShaderNodeMixRGB');l.new(g.outputs[0],mix.inputs[0]);l.new(r.outputs[0],mix.inputs[1]);mix.inputs[2].default_value=rgb((142,113,81));e=n.new('ShaderNodeEmission');l.new(mix.outputs[0],e.inputs[0]);out=n.new('ShaderNodeOutputMaterial');l.new(e.outputs[0],out.inputs[0]);return m
...
def apply(s):
 ...
 co=n.new('ShaderNodeNewGeometry');ns=n.new('ShaderNodeTexNoise');ns.inputs['Scale'].default_value=5.7;ns.inputs['Detail'].default_value=.7;ns.inputs['Roughness'].default_value=.45;l.new(co.outputs['Position'],ns.inputs['Vector']);b=n.new('ShaderNodeBump');b.inputs['Distance'].default_value=.12;b.inputs['Strength'].default_value=.65;l.new(ns.outputs['Fac'],b.inputs['Height']);dp=n.new('ShaderNodeVectorMath');dp.operation='DOT_PRODUCT';dp.inputs[1].default_value=(.47,-.55,.69);l.new(b.outputs['Normal'],dp.inputs[0]);mp=n.new('ShaderNodeMapRange');mp.clamp=True;mp.inputs['From Min'].default_value=.47;mp.inputs['From Max'].default_value=.91;mp.inputs['To Min'].default_value=.45;mp.inputs['To Max'].default_value=1.55;l.new(dp.outputs['Value'],mp.inputs[0]);mix=n.new('ShaderNodeMixRGB');mix.blend_type='MULTIPLY';mix.inputs[0].default_value=.52;l.new(pigment,mix.inputs[1]);l.new(mp.outputs[0],mix.inputs[2]);l.new(mix.outputs[0],em.inputs[0]);gain=n.new('ShaderNodeMixRGB');gain.blend_type='MULTIPLY';gain.inputs[0].default_value=1;gain.inputs[2].default_value=(1.12,1.12,1.12,1);l.new(mix.outputs[0],gain.inputs[1]);l.new(gain.outputs[0],em.inputs[0])
 ...
 verts=[];faces=[];mi=[];count=0;anchor=None;centers=[(rng.uniform(-7.6,7.6),rng.uniform(-9,33),rng.uniform(.25,1.7),rng.uniform(.2,.9)) for _ in range(105)]
 for i in range(23500):
  if rng.random()<.7:
   cx,cy,rx,ry=rng.choice(centers);x=rng.gauss(cx,rx);y=rng.gauss(cy,ry)
  else:x=rng.uniform(-7.8,7.8);y=rng.uniform(-9.5,34)
  if i%5 and anchor is not None and rng.random()<.82:
   x=anchor[0]+rng.gauss(0,.09);y=anchor[1]+rng.gauss(0,.12)
  else:anchor=(x,y)
  ...
  h=r*rng.uniform(.20,.55);N=rng.choice([4,5,6]);angle=rng.random()*math.tau;elong=rng.uniform(.65,1.6);cidx=rng.choices(range(5),[3,2,5,1,3])[0];start=len(verts)
  for j in range(N):
  ...
  for j in range(N):
   bx,by,bz=verts[start+j];f=rng.uniform(.35,.75);verts.append((x+(bx-x)*f+r*.08,y+(by-y)*f-r*.1,z+h*rng.uniform(.65,1.05)))
  for j in range(N):faces.append((start+j,start+(j+1)%N,start+N+(j+1)%N,start+N+j));mi.append(cidx)
  faces.append(tuple(start+N+j for j in range(N)));mi.append(cidx)
  count+=1
 ...
 return {'native_grains':count,'faces':len(faces),'grain_radius_m':[.012,.10],'retained_base_pigment':base.name,'priority':'open ground surface; cracks currently baseline for isolation'}
