...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/alley-rust-143';bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/alley-rust-140/scene.blend'));h=bpy.context.scene.objects['Architecture | gangway_single_Y_8m'];tri=[];weights=[];total=0
...
ob=bpy.data.objects.new('Area samples',me);s.collection.objects.link(ob);m=bpy.data.materials.new('Native coverage');m.use_nodes=True;n=m.node_tree.nodes;l=m.node_tree.links;n.clear();a=n.new('ShaderNodeAttribute');a.attribute_name='sample_position';noise=n.new('ShaderNodeTexNoise');noise.inputs['Scale'].default_value=5.2;noise.inputs['Detail'].default_value=3;noise.inputs['Roughness'].default_value=.72;noise.inputs['Distortion'].default_value=.22;l.new(a.outputs['Vector'],noise.inputs['Vector']);first=n.new('ShaderNodeMath');first.operation='GREATER_THAN';first.inputs[1].default_value=.546;l.new(noise.outputs['Fac'],first.inputs[0]);second=n.new('ShaderNodeTexNoise');second.noise_dimensions='4D';second.inputs['W'].default_value=8.31;second.inputs['Scale'].default_value=7.1;second.inputs['Detail'].default_value=2.5;second.inputs['Roughness'].default_value=.7;second.inputs['Distortion'].default_value=.28;l.new(a.outputs['Vector'],second.inputs['Vector']);bias=n.new('ShaderNodeMath');bias.operation='MULTIPLY';bias.inputs[1].default_value=.02;l.new(first.outputs[0],bias.inputs[0]);score=n.new('ShaderNodeMath');score.operation='ADD';l.new(second.outputs['Fac'],score.inputs[0]);l.new(bias.outputs[0],score.inputs[1]);em=n.new('ShaderNodeEmission');l.new(score.outputs[0],em.inputs[0]);out=n.new('ShaderNodeOutputMaterial');l.new(em.outputs[0],out.inputs['Surface']);me.materials.append(m)
...
pix=list(bpy.data.images.load(str(O/'coverage-field.png'),check_existing=False).pixels);values=[pix[((y*4+2)*256+x*4+2)*4]for y in range(64)for x in range(64)];sv=sorted(values);q=sv[int(len(sv)*.95)]
(O/'coverage-sampling.json').write_text(json.dumps({'samples':4096,'total_modeled_surface_area_m2':total,'threshold_for5percent':q,'coverage_at620':sum(v>.620 for v in values)/len(values),'method':'Area-weighted triangle samples evaluated through the identical native Noise shader; all modeled left Y arm/stem surfaces, including occluded backs; not camera-pixel weighting.'},indent=2))
