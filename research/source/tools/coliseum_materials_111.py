...
def masonry(role='wall'):
 m=bpy.data.materials.new('111 Painted masonry '+role);m.use_nodes=True;n=m.node_tree.nodes;l=m.node_tree.links;n.clear()
 def node(t,label=''):
 ...
 pigment=noise(.38,'Connected worn pigment islands');mask=math('MULTIPLY',ramp(pigment,.63,.72),.38);color=mix(mask,color,rgba('242039'))
 ...
 flowvec=node('ShaderNodeVectorMath','Water follows gravity');flowvec.operation='MULTIPLY';l.new(pos,flowvec.inputs[0]);flowvec.inputs[1].default_value=(1.7,1.7,.12)
 flow=noise(1.9,'Grime branches',flowvec.outputs[0],3)
 broken=ramp(flow,.40,.63);pathmask=math('MULTIPLY',paths,broken)
 pathmask=math('MULTIPLY',pathmask,math('ADD',.35,math('MULTIPLY',bandmask,.65)))
