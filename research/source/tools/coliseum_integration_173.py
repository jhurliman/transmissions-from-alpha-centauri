"""Full native scene carrying the independently retained 172 primary chroma treatment."""
import bpy,sys,json,time,hashlib,array
from pathlib import Path
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-173';O.mkdir(parents=True,exist_ok=True)
...
if 'render'in sys.argv:
    ...
    raise SystemExit
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-168/scene.blend'));s=bpy.context.scene;before=fingerprint(s);mats=material_snapshot()
# Apply only the reviewed primary hue treatment to the retained full scene.
...
(O/'preservation.json').write_text(json.dumps({'source':'retained168','objects':len(before),'changes':changes,'geometry_normals_camera_lights_transforms_and_existing_material_graphs_unchanged':True,'new_face_attributes_only':[]},indent=2))
