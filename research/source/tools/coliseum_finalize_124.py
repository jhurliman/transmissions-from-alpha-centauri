"""124 shared architectural revisions and three true aperture studies."""
import bpy,json,sys,time
from pathlib import Path
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'))
from coliseum_columns_124 import apply as columns
from coliseum_arch_ratio_124 import apply as apertures
...
args=sys.argv[sys.argv.index('--')+1:];label=args[0];reduction={'A':.05,'B':.10,'C':.20}[label]
O=R/'art/studies/coliseum-124'/label;O.mkdir(parents=True,exist_ok=True)
...
aa=apertures(C,reduction);cc=columns(C);cc=cc[1] if isinstance(cc,tuple) else cc
audit={'apertures':aa,'columns':cc,'armature':armature(C),'cornice':cornice(C)}
