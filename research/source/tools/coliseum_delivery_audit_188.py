"""Read-only fresh188scene/kit geometry, normals, materials and dependency audit."""
import bpy,json,hashlib,array,sys
from pathlib import Path
R=Path.cwd();O=R/'art/studies/coliseum-188';sys.path.insert(0,str(R/'tools'))
...
A=read('art/studies/coliseum-188/scene.blend');B=read('art/studies/coliseum-188/kit.blend');out={}
for key,d in [('scene188',A),('kit188',B)]:out[key]={k:v for k,v in d.items()if k!='stamp'}
assert A['contact_ink']==B['contact_ink']
out['portable_contact_ink_matches_scene']=True
a=A['stamp'];b=B['stamp'];out['kit188_vs_scene188']={'expected_objects':len(a),'actual_objects':len(b),'missing':sorted(set(a)-set(b)),'extra':sorted(set(b)-set(a)),'changed':{k:[f for f,vv in v.items()if vv!=b.get(k,{}).get(f)]for k,v in a.items()if v!=b.get(k)}}
assert not out['kit188_vs_scene188']['missing'] and not out['kit188_vs_scene188']['extra'] and not out['kit188_vs_scene188']['changed']
