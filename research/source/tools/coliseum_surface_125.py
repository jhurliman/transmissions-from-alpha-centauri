...
def apply(C,strength=.7):
 ...
 for ob in C.objects:
  ...
  for slot in ob.material_slots:
   ...
   if old not in cache:
    m=old.copy();m.name='125 Ledge mineral '+old.name;cache[old]=m;n,l=m.node_tree.nodes,m.node_tree.links
    ...
    near=0
    for height in [21.06,39.39,57.72,72.2]:
     d=calc('SUBTRACT',height,z);below=calc('MULTIPLY',smooth(d,-.32,.12),calc('SUBTRACT',1,smooth(d,.4,3.7)));near=calc('MAXIMUM',near,below)
    stretched=vec('MULTIPLY',pos,(1.4,1.4,.28));branch=noise(stretched,1.45,3);edge=noise(pos,5.8,2);grain=noise(pos,13,1)
    mask=calc('MULTIPLY',near,smooth(calc('ADD',branch,calc('MULTIPLY',calc('SUBTRACT',edge,.5),.42)),.44,.62));mask=calc('MULTIPLY',mask,strength)
    mix=node('ShaderNodeMixRGB','125 Broken mineral deposits below actual floor bands');mix.blend_type='MULTIPLY';l.new(mask,mix.inputs[0]);l.new(color,mix.inputs[1]);mix.inputs[2].default_value=(.63,.60,.68,1);color=mix.outputs[0]
    fleck=calc('MULTIPLY',calc('MULTIPLY',near,smooth(grain,.57,.72)),strength*.35)
    q=node('ShaderNodeMixRGB','125 Mineral grit retains masonry light');q.blend_type='MULTIPLY';l.new(fleck,q.inputs[0]);l.new(color,q.inputs[1]);q.inputs[2].default_value=(1.30,1.24,1.16,1);l.new(q.outputs[0],em.inputs[0])
   slot.link='OBJECT';slot.material=cache[old];rows.append({'object':ob.name,'material':cache[old].name})
 return {'strength':strength,'materials':len(cache),'assignments':len(rows),'coordinates':'Inverse authored transform of115 Original world position','source_ledge_heights':[21.06,39.39,57.72,72.2],'scope':'wall tower and band materials only; preserve original light response and all geometry','rows':rows}
