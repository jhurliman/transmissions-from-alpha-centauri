...
O=R/'art/studies/coliseum-187/geometry'
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-173/scene.blend'));s=bpy.context.scene;C=bpy.data.collections['110 Coliseum detailed front ruin'];dg=bpy.context.evaluated_depsgraph_get();original,world,unpack=mapping();names=json.load(open(R/'config/coliseum-left-course-187.json'))['targets'];core=bpy.data.materials['151 Warm violet exposed masonry core']
...
for name in names:
 ...
 if before['crossings']or before['nonmanifold']:row['inherited_source_topology_warning']=True
 # Exact unaffected source triangles must remain; scope defined by convex cutter.
 protectedids=[];anchorids=[]
 vv=[]
 offset=.35 if name.endswith('profile0') else .12 if name.endswith('profile3') else .18 if 'sill wall' in name else 0.
 for z in [39.75,43.15]:
  la=16.60-.28*202+.10*(z-41)+offset
  ba=212.4-.35*18+.05*(z-41)
  xl=(la+.28*ba)/(1-.28*.35);xr=21.3
  for x,y in [(la+.28*195,195),(xr,195),(xr,ba+.35*xr),(xl,ba+.35*xl)]:vv.append(iv@Vector((x,y,z)))
 ff=[(3,2,1,0),(4,5,6,7),(0,1,5,4),(1,2,6,5),(2,3,7,6),(3,0,4,7)]
 cm=bpy.data.meshes.new('187 variable-depth negative solid');cm.from_pydata(vv,[],ff);bm=bmesh.new();bm.from_mesh(cm);bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(cm);bm.free();cut=bpy.data.objects.new('187 temporary negative solid',cm);C.objects.link(cut);ctree=BVHTree.FromPolygons([v.co.copy()for v in cm.vertices],[tuple(p.vertices)for p in cm.polygons]);tmp=bpy.data.objects.new('187 temporary target',src.copy());C.objects.link(tmp);mod=tmp.modifiers.new('187 subtractive compound crown','BOOLEAN');mod.operation='DIFFERENCE';mod.solver='EXACT';mod.object=cut;bpy.context.view_layer.objects.active=tmp;bpy.ops.object.modifier_apply(modifier=mod.name);bpy.data.objects.remove(cut,do_unlink=True);me=tmp.data;me.update()
 ...
 # One numerical cleanup on edges not inherited verbatim; unchanged cutter.
 oldedges={tuple(sorted((tuple(src.vertices[e.vertices[0]].co),tuple(src.vertices[e.vertices[1]].co))))for e in src.edges}
 bm=bmesh.new();bm.from_mesh(me)
 newedges=[e for e in bm.edges if tuple(sorted(tuple(v.co)for v in e.verts))not in oldedges]
 bmesh.ops.dissolve_degenerate(bm,edges=newedges,dist=1e-6/max(M.to_3x3().col[j].length for j in range(3)))
 bm.to_mesh(me);bm.free();me.update()
 after=health(me,M);
 # All triangles disjoint from cutter AABB are protected, a conservative exact gate.
 lo=Vector(tuple(min(v[j]for v in vv)for j in range(3)));hi=Vector(tuple(max(v[j]for v in vv)for j in range(3)))
 protectedids=[p.index for p in src.polygons if any(max(verts[i][j]for i in p.vertices)<lo[j]-1e-5 or min(verts[i][j]for i in p.vertices)>hi[j]+1e-5 for j in range(3))]
 row['after']=after;
 def pairkeys(mesh):
  mesh.calc_loop_triangles();out=set()
  for pair in robust_crossings(types.SimpleNamespace(data=mesh,matrix_world=M)):
   out.add(tuple(sorted(tuple(sorted(tuple(mesh.vertices[v].co)for v in mesh.loop_triangles[i].vertices))for i in pair)))
  return out
 oldpairs=pairkeys(src);newpairs=pairkeys(me);row['new_crossing_pair_count']=len(newpairs-oldpairs);row['inherited_pair_count_retained']=len(newpairs&oldpairs)
 row['removed_volume_m3']=before['volume']-after['volume'];newkeys={tuple(sorted(tuple(me.vertices[i].co)for i in p.vertices)):p.index for p in me.polygons};missing=[pi for pi in protectedids if tuple(sorted(tuple(src.vertices[i].co)for i in src.polygons[pi].vertices))not in newkeys];row['protected_missing']=missing;row['missing_peak_anchor_vertices']=[i for i in anchorids if tuple(verts[i])not in {tuple(v.co)for v in me.vertices}]
 if row['new_crossing_pair_count']or after['nonmanifold']or after['zero_faces']or after['components']!=before['components']or row['removed_volume_m3']<=0 or missing or row['missing_peak_anchor_vertices']:row['held']='Topology, volume or protected-face gate'
 # Restore original coordinates and all surviving source-face assignments.
 pa=me.attributes['115 Original world position'];me.materials.append(core);ci=len(me.materials)-1;tag=me.attributes.new('187 Exposed crown core','FLOAT','FACE');custom=[n.vector.copy()for n in me.corner_normals];corecount=0;retainedcount=0
 ...
 for p in samples:
  ...
  if h[3]>1e-5:outside.append(float((p-h[0]).dot(h[1])))
...
audit={'source':'173','targets':rows,'accepted_cpu':not any('held'in r for r in rows)and len(payload)==6,'seconds':time.time()-start,'normal_preservation':'Quantified after Blender custom-normal storage; not claimed bit-exact.'};(O/'audit.json').write_text(json.dumps(audit,indent=2))
if len(payload)==6:
 ...
 bpy.ops.wm.save_as_mainfile(filepath=str(O/'fixed-shape-technical.blend'))
print('187 FINAL',json.dumps(audit),flush=True)
