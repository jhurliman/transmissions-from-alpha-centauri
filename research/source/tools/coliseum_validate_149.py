...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-149'
a=np.array(Image.open(R/'art/studies/coliseum-148/main-4k.png').convert('RGB')).astype('int16')
...
r={'baseline':'148 main-4k.png','native_size':[3840,2885],'landmark_region':[1250,300,2500,1300],'outside_region_pixels_over_4':int(((d>4)&outside).sum()),'outside_region_pixels_over_12':int(((d>12)&outside).sum()),'outside_region_max_channel_delta':int(d[outside].max()),'note':'Region includes whole visible landmark. Geometry preservation is separately recorded; raster differences outside it require inspection.'}
for name,box in [('crown',(1900,400,2120,610)),('cornice',(1680,710,1835,840))]:
