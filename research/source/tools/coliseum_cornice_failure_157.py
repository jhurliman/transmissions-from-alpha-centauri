...
TARGETS=['COL110 T2 band10 profile'+str(i)for i in range(5)]+['COL110 T2 band11 profile'+str(i)for i in range(4)]+['COL110 U10 sill wall']
...
def apply(C):
    dg=bpy.context.evaluated_depsgraph_get();rows=[];skipped=[]
    ...
    for name in TARGETS:
        ...
        if 'profile' in name:
            ...
            lo,left,right,hi=spec[:4];xs=[left-.45,left-.27,left-.08,left+.18,right-.23,right,right+.12,right+.45];offset=spec[4]
            ...
            xs=[18.25,18.65,18.80,19.28,19.44,20.35,20.48,20.84,20.98,21.12];offset=4.5
            lows=[41.20]*len(xs);highs=[41.40,41.60,42.30,42.20,43.10,42.96,42.10,42.00,41.60,41.30];depths=[.04,.22,.75,.95,1.75,1.60,.80,.45,.16,.04]
        source_x=[(M@v.co).x for v in me.vertices]
        if max(source_x)<min(xs) or min(source_x)>max(xs):
            skipped.append({'object':name,'reason':'Entire source lies outside the authored short course-end cutter; preserve native object unchanged','source_x_range':[min(source_x),max(source_x)],'cutter_x_range':[min(xs),max(xs)]})
        ...
        # A staggered end can meet the adjacent bay's bounding box without
        # intersecting its curved masonry. Keep the original native object
        # when the exact render triangles are unchanged; this is not a cut.
        me.calc_loop_triangles()
        source_surface={tuple(sorted(tuple(src[i])for i in tri))for tri in tris}
        result_surface={tuple(sorted(tuple(me.vertices[i].co)for i in tri.vertices))for tri in me.loop_triangles}
        if result_surface==source_surface and len(me.loop_triangles)==len(tris):
            skipped.append({'object':name,'reason':'Boolean leaves every source render triangle bit exact; native object preserved unchanged','source_triangles':len(tris),'result_triangles':len(me.loop_triangles)})
            bpy.data.objects.remove(tmp,do_unlink=True);bpy.data.meshes.remove(me)
            continue
    ...
    return {'source':'156','targets':rows,'skipped_unchanged':skipped,'operation':'Native DIFFERENCE only; stepped projecting courses with retained attached returns','references':['UCL-01','UCL-02','DP-03'],'no_constructive_additions':True}
