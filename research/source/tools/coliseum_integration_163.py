"""Full native scene carrying the independently retained162 facade grouping."""
import bpy,sys,json,time,hashlib,array
from pathlib import Path
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-163';O.mkdir(parents=True,exist_ok=True)
...
if 'render'in sys.argv:
    ...
    raise SystemExit
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-159/scene.blend'));s=bpy.context.scene;before=fingerprint(s);mats=material_snapshot()
# Reapply the reviewed module to the corrected159 baseline, rather than adopting
# the proof's camera border settings or pre161 embedded visibility texts.
...
d=apply(C);d['visibility']=guards(s,True);after=fingerprint(s);ma=material_snapshot();changes={k:[f for f in v if v[f]!=after[k][f]]for k,v in before.items()if v!=after[k]};allowed={r['object']for r in d['assignments']}
...
(O/'preservation.json').write_text(json.dumps({'source':'corrected159','objects':len(before),'changes':changes,'geometry_normals_camera_lights_transforms_and_existing_material_graphs_unchanged':True,'new_face_attributes_only':[r['object']for r in d['receiving_masks']]},indent=2))
