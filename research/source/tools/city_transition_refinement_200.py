 ...
 for n in m.node_tree.nodes:
  if n.type=='VALTORGB'and n.label=='133 Broken-building shade families':
   for el in n.color_ramp.elements:
    c=el.color;el.color=(c[0]*.48,c[1]*.53,c[2]*.60,c[3])
 return m
...
def _cut(ob,center,normal,outline,depth,label,core):
 ...
 cutmat=next(i for i,m in enumerate(me.materials)if m==core)
...
def _outline(rng,width,height):
 # Irregular stepped fracture, deliberately not a radial oval pocket.
 template=[(-.10,-.50),(-.34,-.34),(-.24,-.17),(-.49,-.03),(-.32,.17),(-.18,.12),(-.20,.37),(-.01,.50),(.10,.27),(.35,.34),(.47,.11),(.20,-.03),(.27,-.28),(.03,-.23)]
 flip=rng.choice((-1,1));return[(x*width*flip*rng.uniform(.92,1.08),z*height*rng.uniform(.94,1.06))for x,z in template]
...
def build(scene):
 ...
 coreold=bpy.data.materials['133 cool fractured concrete'];core=_core_material(coreold)
 for ob in sorted(C.objects,key=lambda o:o.name):
  ...
  if not candidates:continue
  # One connected fracture family starts at an actually visible broken crown.
  high=max(candidates,key=lambda p:p['point'][2]);n=Vector(high['normal']);u=n.cross(Vector((0,0,1))).normalized()
  height=rng.uniform(1.05,1.70);width=rng.uniform(.48,1.02)
  crown=Vector(high['point'])+Vector((0,0,-height*.12));features=[]
  features.append(_cut(ob,crown,n,_outline(rng,width,height),.43,'connected jagged crown loss',core))
  # The thin descending split belongs to that same loss, not a detached face-center spot.
  if len(samples)>45:
   c=crown-Vector((0,0,height*.68))+u*rng.uniform(-.10,.10)
   profile=[(-.020,.78),(.055,.48),(.028,.21),(.108,.01),(.030,-.28),(.009,-.79),(-.046,-.34),(-.022,-.05),(-.079,.20),(-.024,.44)]
   features.append(_cut(ob,c,n,profile,.115,'descending connected fracture',core))
  # Tiny unequal secondary bite widens one existing broken shoulder on larger remnants.
  if len(samples)>400:
   c=crown+u*(width*.43)+Vector((0,0,.10));features.append(_cut(ob,c,n,_outline(rng,width*.48,height*.54),.36,'adjoining fracture shoulder',core))
  health=_closed(ob)
  if health['nonmanifold_edges']:raise RuntimeError((ob.name,health))
  rows.append(dict(object=ob.name,visible_ray_samples=len(samples),features=features,health=health,before_hash=originals[ob.name],after_hash=_mesh_digest(ob)))
 return dict(version=200,source='beam-rust-197',target_collection=CNAME,objects_with_geometry_edits=len(rows),material_objects=sum(o.type=='MESH'for o in C.objects),features=sum(len(r['features'])for r in rows),rows=rows,old_material_graphs_changed=False,all_other_collections_unchanged=True,object_transforms_unchanged=True,references=['UCL-01','UCL-02','DP-03'],limits='V2 removes isolated oval pits; connected crown losses and descending fractures only. Visibility placement sampled at4px; native proof required. No road/new rubble placement changes.')
...
def apply(scene):
 ...
 with bpy.data.libraries.load(str(O/'payload.blend'),link=False)as(src,dst):
  dst.meshes=list(j['meshes']);dst.materials=list(j['materials'])
