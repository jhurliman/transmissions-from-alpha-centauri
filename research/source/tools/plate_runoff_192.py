...
def apply(scene):
 ...
 material=bpy.data.materials['189 Fastener oxide translucent pigment'].copy();material.name='192 Smooth translucent rusty water'
 ...
 for p in plates:
  ...
  for b in bolts:
   ...
   rows_root=[]
   for j in range(11):
    t=j/10;theta=math.radians(12+76*t);row=[]
    for k in range(5):
     u=k/4;ri=.74 if washer else .40;ro=.97 if washer else .86;r=radius*(ri+(ro-ri)*u)*(1+.025*math.sin(t*15+t*15+radius*37));q=seat['p']+(across*(outer_sign*math.cos(theta))+down*math.sin(theta))*r
     alpha=.92*(math.sin(math.pi*u)**.55)*(math.sin(math.pi*t)**.40);row.append((seat_hit(q),(.34,.095,.030,alpha)))
   ...
   for line in range(count):
    if washer:offset=outer_sign*radius*([.84,.64][line]if count==2 else .84);drop=radius*([.60,.82][line]if count==2 else .60)
    else:offset=outer_sign*a_extent*(1.12 if line==0 else .88);drop=v_extent*(.72 if line==0 else 1.05)
    bolt_u=(b['p']-p['p']).dot(across);clamped_u=max(umin+.036*H,min(umax-.036*H,bolt_u+offset));offset=clamped_u-bolt_u
    ...
    half=H*rng.uniform(.0235,.0280)*(1 if line==0 else .8);drift=rng.uniform(-.15,.15)*half;ph=rng.uniform(0,math.tau);prior=len(faces);prior_vertex=len(verts)
    # Outer diluted wash and a narrower oxide-rich stream share the same origin.
    for role,width_factor,pigment,peak,zoff in [('soft outer wash',1,(.29,.095,.040),.76,.0020),('narrow oxide core',.52,(.16,.042,.022),.96,.0024)]:
    ...
    entry['lines'].append(dict(root=list(root),end=list(root+down*length),length=length,length_as_plate_height=length/H,nominal_target=.15*H,lower_reaches_bottom=lower and abs(start_v+length-(bottom-.0012))<1e-5,bottom_clearance=bottom-(start_v+length),outer_width=2*half,core_width=2*half*.52,geometry_bottom_clearance=bottom-max((v-p['p']).dot(down)for v in verts[prior_vertex:])if len(verts)>prior_vertex else None,faces=len(faces)-prior))
