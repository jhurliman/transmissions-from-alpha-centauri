"""Append current kit into an empty native scene and render coherent representative proofs."""
...
def signature(ob):
 ...
 if ob.type=='MESH':
  ev=ob.evaluated_get(bpy.context.evaluated_depsgraph_get());me=ev.to_mesh();a=array.array('f',[0])*(len(me.vertices)*3);me.vertices.foreach_get('co',a);b=array.array('i',[0])*len(me.loops);me.loops.foreach_get('vertex_index',b);d['evaluated_geometry']=hashlib.sha256(a.tobytes()+b.tobytes()).hexdigest();ev.to_mesh_clear()
 return d
before={o.name:signature(o)for o in C.all_objects};bpy.ops.wm.read_factory_settings(use_empty=True);s=bpy.context.scene
with bpy.data.libraries.load(str(kit),link=False)as(src,dst):dst.collections=['110 Coliseum detailed front ruin']
...
bpy.context.view_layer.update();after={o.name:signature(o)for o in C.all_objects};changes=[k for k,v in before.items() if v!=after.get(k)];assert not changes,changes
...
regions={'bay-tower':(1630,430,2070,975),'crown':(1410,350,2350,630)}
...
(O/'performance.json').write_text(json.dumps({'four_proof_render_seconds':time.time()-t,'native_frame_resolution':resolution,'regions':regions},indent=2))
