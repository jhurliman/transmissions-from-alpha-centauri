...
def apply(C,strength=.85,core=True,deposits=True):
 ...
 for ob in C.all_objects:
  ...
  for slot in ob.material_slots:
   old=slot.material
   if not old or not old.use_nodes or old.get('131 surface'):continue
   ...
   if old not in cache:
    m=old.copy();m.name='131 Attached mineral '+old.name;cache[old]=m;m['131 surface']=True;n,l=m.node_tree.nodes,m.node_tree.links
    def node(t,label=''):
    ...
    def mix(f,a,b,mode='MULTIPLY',label=''):
     ...
     return q.outputs[0]
    at=node('ShaderNodeAttribute','131 Original geometry-attached mineral position');at.attribute_name='115 Original world position';comb=node('ShaderNodeCombineXYZ')
    ...
    if is_core:
     ...
     color=mix(calc('MULTIPLY',islands,.60*strength),color,(.57,.54,.65,1),label='131 Connected exposed mineral matrix')
     light=calc('MULTIPLY',calc('SUBTRACT',1,smooth(edge,.27,.40)),.6*strength);color=mix(light,color,(1.42,1.28,1.18,1),label='131 Broken chalky mineral grains')
     pits=calc('MULTIPLY',smooth(fine,.62,.76),.55*strength);color=mix(pits,color,(.38,.36,.46,1),label='131 Fine fracture pores at native pixel scale')
    elif deposits:
     ...
     d=node('ShaderNodeAttribute','131 Keep deep arch returns quiet');d.attribute_name='120 Actual arch tunnel depth';gate=calc('SUBTRACT',1,smooth(d.outputs['Fac'],.12,.35));mask=calc('MULTIPLY',calc('MAXIMUM',calc('MULTIPLY',mask,.63),calc('MULTIPLY',source,.44)),calc('MULTIPLY',strength,gate))
     color=mix(mask,color,(.38,.34,.46,1),label='131 Finite ledge and damage-fed runoff')
     flecks=calc('MULTIPLY',calc('MULTIPLY',source,smooth(edge,.58,.72)),.20*strength);color=mix(flecks,color,(1.25,1.12,1.06,1),label='131 Mineral deposit interruptions around sources')
