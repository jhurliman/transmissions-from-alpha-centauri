...
R=Path(__file__).resolve().parents[1];O=R/'art/reviews/xenon-031';O.mkdir(parents=True,exist_ok=True)
...
C=bpy.data.collections.new('031 Distant large service infrastructure');s.collection.children.link(C)
...
s.render.use_freestyle=False;s.view_settings.view_transform='AgX';s.view_settings.look='None';s.view_settings.exposure=0
s.cycles.samples=64;s.cycles.use_denoising=True
for ob in s.objects:
 if ob.type=='LIGHT':
  if ob.data.type=='AREA':ob.hide_render=True
  if ob.data.type=='SUN':ob.data.energy=1.7;ob.data.color=(1.0,.83,.67);ob.data.angle=.14
if s.world and s.world.use_nodes:
 for node in s.world.node_tree.nodes:
  if node.type=='BACKGROUND' and node.name=='Background':node.inputs['Color'].default_value=(.42,.49,.62,1);node.inputs['Strength'].default_value=2.0
