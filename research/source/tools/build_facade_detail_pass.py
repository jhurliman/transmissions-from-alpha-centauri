...
def service_bay():
 ...
 box('Chase recessed back',(-1.57,.76,1.66),(1.60,.05,3.32),dark)
 for x in [-2.37,-.77]:
  box('Chase side return',(x,.36,1.66),(.035,.80,3.32),paint)
  box('Chase edge strip',(x,0,1.66),(.065,.04,3.32),pale)
 for z in [.03,3.29]:box('Chase end closure',(-1.57,.36,z),(1.6,.80,.055),steel)
...
tail=tail.replace("for key,col in proofs.items():\n",'''for key,col in proofs.items():
 if key=='panel_layouts':continue
 bpy.data.objects['Studio floor'].hide_render=key=='canopy_underside'
 if key=='panel_layouts':
  target=Vector((0,0,1.05));delta=Vector((2,-15,3));d.ortho_scale=10.2
 elif key=='canopy_underside':
  target=Vector((0,-.6,1.0));delta=Vector((3,-5,-1.0));d.ortho_scale=5.8
 else:
...
''')
# Inspection light illuminates real underside geometry; disabled for service bay.
tail=tail.replace("for key,col in proofs.items():\n", """inspection_data=bpy.data.lights.new('Underside inspection light','AREA');inspection_data.energy=280;inspection_data.size=3
inspection=bpy.data.objects.new('Underside inspection light',inspection_data);C.objects.link(inspection);inspection.location=(0,-3,.2);inspection.rotation_euler=(Vector((0,-.5,1.5))-inspection.location).to_track_quat('-Z','Y').to_euler()
for key,col in proofs.items():
 inspection.hide_render=key!='canopy_underside'
""")
exec(tail)
