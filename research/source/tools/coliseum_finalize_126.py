"""126 user-directed grouped arcade, taller platforms and visible upper bases."""
import bpy,json,sys,time
from pathlib import Path
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-126';sys.path.insert(0,str(R/'tools'))
from coliseum_platforms_126 import apply as platforms
from coliseum_column_bases_126 import apply as bases
from coliseum_outer_rim_126 import apply as rim
from coliseum_groups_126 import apply as grouping
from sky_palette_126 import apply as sky
...
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-125/scene.blend'));s=bpy.context.scene;C=bpy.data.collections['110 Coliseum detailed front ruin'];t=time.time()
audit={'sky':sky(),'platforms':platforms(C),'bases':bases(C),'rim':rim(C),'grouping':grouping(C)};audit['generation_seconds']=time.time()-t;(O/'generation.json').write_text(json.dumps(audit,indent=2,default=str))
