...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-126/crown-deform-v3';sys.path.insert(0,str(R/'tools'))
...
def apply(C):
 ...
 for ni,name in enumerate(NAMES):
  ob=bpy.data.objects[name];old=ob.data;triangulate_render(ob);base=topology(ob);me=ob.data;ctrl=profiles[ni]['original_control_profile'];curves=profiles[ni]['notch_profiles'];curves=[[(u,d*2.0)for u,d in c]for c in curves]
  at=me.attributes.get('118 Recess interior');protected={i for f in me.polygons if at and at.data[f.index].value>.5 for i in f.vertices};pv=[uz(Fi@(ob.matrix_world@me.vertices[i].co))for i in protected];pv=[(u,z)for u,z in pv if z<piece(u,ctrl)-.05]
  ...
  bmesh.ops.subdivide_edges(bm,edges=eligible,cuts=23,use_grid_fill=True);bmesh.ops.triangulate(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();me.update();sub=topology(ob)
  if sub['nonmanifold']or sub['strict_crossings']:ob.data=old;raise RuntimeError('Invalid subdivision '+name+str(sub))
  protected_sub={i for f in me.polygons if me.attributes.get('118 Recess interior') and me.attributes['118 Recess interior'].data[f.index].value>.5 for i in f.vertices if (lambda q: q[1]<piece(q[0],ctrl)-1.5)(uz(Fi@(ob.matrix_world@me.vertices[i].co)))}
  ...
  for v in me.vertices:
   ...
   d=min(max((piece(u,c)for c in curves),default=0),h*.30);t=max(0,min(1,(z-z0)/h));f=t;delta=d*f
  ...
  rows.append({'object':name,'before':base,'subdivided':sub,'after':after,'moved_vertices':moved,'max_author_z_reduction':maxd,'derivative_lower_bound':1-.30,'protected_macro_vertices':len(protected),'protected_subdivided_max_change':protected_change,'method':'x/y fixed in accepted transformed author frame; quintic z fade above protected recess ceiling','new_objects':0});ob['126 monotone crown erosion']=True
 return rows
