...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/reviews/xenon-061'
def mat(name,c):
...
for variant in ['network']:
 ...
 for hostName,partName in [('Architecture | buttress_45','Solid concrete'),('Front-left section instance','Tapered column structural volume')]:
  ...
  subprocess.run(['/tmp/crack060-env/bin/python',str(R/'tools/fracture_mesh_061.py'),str(req),str(res)],check=True)
  mesh=json.loads(res.read_text());verts=[tuple(origin+u*(umin+x)+v*(vmin+y)+n*z) for x,y,z in mesh['vertices']]
  c=mesh_obj('061 unified chamfer cutter',verts,mesh['faces'],cutcol,lip)
 ...
 proof=bpy.data.objects.new('061 actual cut column geometry',me);s.collection.objects.link(proof);proof.matrix_world=transform
 s.world=bpy.data.worlds.new('061 neutral world');s.world.use_nodes=True;s.world.node_tree.nodes['Background'].inputs[0].default_value=(.12,.12,.12,1)
 clay=bpy.data.materials.new('061 neutral geometry proof');clay.use_nodes=True;bs=clay.node_tree.nodes.get('Principled BSDF');bs.inputs['Base Color'].default_value=(.35,.35,.35,1);bs.inputs['Roughness'].default_value=.7;s.view_layers[0].material_override=clay;s.render.use_freestyle=False
 bpy.ops.object.light_add(type='AREA',location=s.camera.location+Vector((-.5,-2,2)));light=bpy.context.object;light.data.energy=900;light.data.shape='DISK';light.data.size=2;light.rotation_euler=(proofs[-1][1]-light.location).to_track_quat('-Z','Y').to_euler();s.render.filepath=str(O/'panel-clay.png');bpy.ops.render.render(write_still=True);(O/'proof-camera.json').write_text(json.dumps({'position':list(s.camera.location),'rotation':list(s.camera.rotation_euler),'projection':s.camera.data.type,'scale':s.camera.data.ortho_scale,'same_as_shaded_panel':True},indent=2))
