...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-126/voxel-chips';sys.path.insert(0,str(R/'tools'))
...
def apply(C):
 ...
 def uz(q):
  ...
  return(a-ac)*75,q.z
 for ni,name in [(1,NAMES[1])]:
  ob=bpy.data.objects[name];old=ob.data;ob.data=old.copy();base=topology(ob);me=ob.data;ctrl=profiles[ni]['original_control_profile'];curves=profiles[ni]['notch_profiles'];curves=[[(u,d*2.0)for u,d in c]for c in curves]
  ...
  for v in me.vertices:
   ...
   d=min(max((piece(u,c)for c in curves),default=0),h*.20);t=max(0,min(1,(z-z0)/h));f=t*t*t*(t*(t*6-15)+10);delta=d*f
  ...
  rows.append({'object':name,'before':base,'subdivided':sub,'after':after,'moved_vertices':moved,'max_author_z_reduction':maxd,'derivative_lower_bound':1-1.875*.20,'protected_macro_vertices':len(protected),'protected_subdivided_max_change':protected_change,'method':'x/y fixed in accepted transformed author frame; quintic z fade above protected recess ceiling','new_objects':0});ob['126 monotone crown erosion']=True
 return rows
