 ...
 for ob in C.objects:
  ...
  for slot in ob.material_slots:
   ...
   if old not in cache:
    ...
    coarse=smooth(noise(pos,.55,2),.51,.64)
    ...
    ledge=calc('MULTIPLY',bandnear,smooth(calc('ADD',branch,calc('MULTIPLY',calc('SUBTRACT',edge,.5),.42)),.40,.57))
    broad=noise(vec('MULTIPLY',pos,(1.,1.,.65)),1.05,2)
    patch=calc('MULTIPLY',junction,smooth(calc('ADD',broad,calc('MULTIPLY',calc('SUBTRACT',edge,.5),.12)),.49,.56))
    mask=calc('MULTIPLY',calc('MAXIMUM',ledge,calc('MULTIPLY',patch,.85)),strength)
    mix=node('ShaderNodeMixRGB','126 Broken mineral deposits below actual floor bands');mix.blend_type='MULTIPLY';l.new(mask,mix.inputs[0]);l.new(color,mix.inputs[1]);mix.inputs[2].default_value=(.48,.44,.55,1);color=mix.outputs[0]
    fleck=calc('MULTIPLY',calc('MULTIPLY',calc('MAXIMUM',bandnear,calc('MULTIPLY',junction,.25)),smooth(grain,.57,.72)),strength*.55)
