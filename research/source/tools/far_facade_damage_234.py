...
def apply(scene):
 ...
 for index,r in enumerate(chosen):
  ...
  face=q.data.polygons[r['face']];rng=random.Random(int(hashlib.sha256((root.name+str(index)+old.name).encode()).hexdigest()[:12],16));fx,fy=rng.choice(r['options']);cx=r['lo'][0]+r['W']*fx;cy=r['lo'][1]+r['H']*fy;kind='crack'if index%3==1 else'spall'
  ...
  if kind=='spall':
   # Unequal torn sheets, with broad broken ends rather than pointed polar ovals.
   rx=min(r['W']*.41,(cx-r['lo'][0])*.86,(r['hi'][0]-cx)*.86);ry=min(r['H']*rng.uniform(.27,.36),rx*rng.uniform(1.65,2.05),(cy-r['lo'][1])*.86,(r['hi'][1]-cy)*.86)
   shapes=[
    [(-.72,.84),(-.08,.94),(.14,.62),(.64,.68),(.86,.29),(.48,.04),(.70,-.34),(.27,-.84),(-.27,-.88),(-.38,-.55),(-.76,-.42),(-.56,.03),(-.85,.33)],
    [(-.80,.65),(-.35,.89),(.29,.81),(.28,.42),(.79,.33),(.64,-.05),(.88,-.44),(.33,-.51),(.17,-.86),(-.42,-.78),(-.53,-.37),(-.83,-.26),(-.61,.24)],
    [(-.83,.47),(-.49,.83),(.18,.89),(.32,.58),(.82,.52),(.67,.08),(.39,-.03),(.68,-.59),(.13,-.85),(-.34,-.78),(-.33,-.37),(-.74,-.49),(-.64,-.02)]
   ];shape=shapes[index%len(shapes)];flip=-1 if rng.random()<.5 else 1;outer=[(cx+flip*(x+rng.uniform(-.055,.055))*rx,cy+(y+rng.uniform(-.055,.055))*ry)for x,y in shape]
   inner=[(cx+(x-cx)*.83,cy+(y-cy)*.84)for x,y in outer]
  else:
   # Directed unequal runs: one principal drift, one larger kink, no alternating cadence.
   half=min(r['H']*.40,(cy-r['lo'][1])*.88,(r['hi'][1]-cy)*.88);runs=[rng.uniform(.06,.16),rng.uniform(.25,.37),rng.uniform(.045,.085),rng.uniform(.15,.25),rng.uniform(.20,.34)];total=sum(runs);runs=[x/total for x in runs];sign=rng.choice([-1,1]);x=cx-sign*r['W']*.075;y=cy+half;pts=[(x,y)];cumulative=0
   slopes=[.10,.07,-.22,.035,.06]
   for j,f in enumerate(runs):
    cumulative+=f;y=cy+half*(1-2*cumulative);x+=sign*r['W']*slopes[j]*rng.uniform(.78,1.12);pts.append((x,y))
   width=min(r['W']*.035,.045);profile=[.06,.58,1.,.62,.38,.025]
   def ribbon(extra):
    left=[];right=[]
    for j,(x,y)in enumerate(pts):
     prev=Vector(pts[max(0,j-1)]);nxt=Vector(pts[min(len(pts)-1,j+1)]);d=(nxt-prev).normalized();n=Vector((-d.y,d.x));w=(width+extra)*profile[j];left.append((x+n.x*w,y+n.y*w));right.append((x-n.x*w,y-n.y*w))
    return left+right[::-1]
   outer=ribbon(min(r['W']*.017,.020));inner=ribbon(0)
