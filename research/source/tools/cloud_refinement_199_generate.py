...
for k,src in enumerate(sorted((R/'art/studies/cloud-082/assets').glob('*-pigment.png'))):
 ...
 activity=np.clip(.57+.36*noise(7,17),.10,1.0)
 gy,gx=np.gradient(d);upper=np.clip(.5+.5*gy,0,1)
 peaks=np.maximum(shoulder-.55,0)**1.2
 contour=d+U*((2.6*shoulder+1.7*lobes+1.15*peaks)*activity*(.40+.90*upper)+.28*fine*(.35+.65*upper))*taper
 ...
 veil1=noise(2.5,15);veil2=noise(1.1,8)
 veils=np.clip((veil1+.35)/1.9,0,1);veils=veils*veils*(3-2*veils)
 # Fine elongated overlapping veils live preferentially near native shoulders,
 # while deep interiors retain calm broad tone. No discrete band thresholds.
 shoulder_zone=np.exp(-np.maximum(d,0)/(11*U))
 tone=.48+.28*(broad-.5)+.20*(layer-.5)+.026*noise(5,20)+.075*(veils-.5)+.030*veil2*(.3+.7*shoulder_zone)
