"""Read-only dense support comparison for the proposed intermediate course-end simplification."""
import bpy
import json
import math
import sys
...
bpy.ops.wm.open_mainfile(filepath=str(R / 'art/studies/coliseum-185/geometry/candidate.blend'))
...
for record in snap:
    for index, stroke in enumerate(record['strokes']):
        ...
        if tested:
            ...
            total += tested
result = {'source_digest': inventory['source_drawing_digest'], 'candidate': '185/geometry/candidate.blend',
