...
O=R/'art/studies/coliseum-185/geometry'
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-173/scene.blend'));s=bpy.context.scene;C=bpy.data.collections['110 Coliseum detailed front ruin'];dg=bpy.context.evaluated_depsgraph_get();original,world,unpack=mapping();names=json.load(open(R/'config/coliseum-course-simplification-185.json'))['targets'];core=bpy.data.materials['151 Warm violet exposed masonry core']
...
for name in names:
 ...
 vv=[]
 for z in [38.,45.]:
  la=17.55-.1907894737*203.4+.12*(z-41)
  ra=25.65+.2727272727*213.2-.07*(z-41)
  ba=211-.9565217391*19+.06*(z-41)
  xl=(la+.1907894737*ba)/(1-.1907894737*.9565217391)
  xr=(ra-.2727272727*ba)/(1+.2727272727*.9565217391)
  for x,y in [(la+.1907894737*180,180),(ra-.2727272727*180,180),(xr,ba+.9565217391*xr),(xl,ba+.9565217391*xl)]:vv.append(iv@Vector((x,y,z)))
 ff=[(3,2,1,0),(4,5,6,7),(0,1,5,4),(1,2,6,5),(2,3,7,6),(3,0,4,7)]
 cm=bpy.data.meshes.new('185 variable-depth negative solid');cm.from_pydata(vv,[],ff);bm=bmesh.new();bm.from_mesh(cm);bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(cm);bm.free();cut=bpy.data.objects.new('185 temporary negative solid',cm);C.objects.link(cut);ctree=BVHTree.FromPolygons([v.co.copy()for v in cm.vertices],[tuple(p.vertices)for p in cm.polygons]);tmp=bpy.data.objects.new('185 temporary target',src.copy());C.objects.link(tmp);mod=tmp.modifiers.new('185 subtractive compound crown','BOOLEAN');mod.operation='DIFFERENCE';mod.solver='EXACT';mod.object=cut;bpy.context.view_layer.objects.active=tmp;bpy.ops.object.modifier_apply(modifier=mod.name);bpy.data.objects.remove(cut,do_unlink=True);me=tmp.data;me.update()
 ...
 row['after']=after;row['removed_volume_m3']=before['volume']-after['volume'];newkeys={tuple(sorted(tuple(me.vertices[i].co)for i in p.vertices)):p.index for p in me.polygons};missing=[pi for pi in protectedids if tuple(sorted(tuple(src.vertices[i].co)for i in src.polygons[pi].vertices))not in newkeys];row['protected_missing']=missing;row['missing_peak_anchor_vertices']=[i for i in anchorids if tuple(verts[i])not in {tuple(v.co)for v in me.vertices}]
 if after['crossings']or after['nonmanifold']or after['zero_faces']or after['components']!=before['components']or row['removed_volume_m3']<=0 or missing or row['missing_peak_anchor_vertices']:row['held']='Topology, volume or protected-face gate'
 # Restore original coordinates and all surviving source-face assignments.
 pa=me.attributes['115 Original world position'];me.materials.append(core);ci=len(me.materials)-1;tag=me.attributes.new('185 Exposed crown core','FLOAT','FACE');custom=[n.vector.copy()for n in me.corner_normals];corecount=0;retainedcount=0
...
audit={'source':'173','targets':rows,'accepted_cpu':not any('held'in r for r in rows)and len(payload)==5,'seconds':time.time()-start,'normal_preservation':'Quantified after Blender custom-normal storage; not claimed bit-exact.'};(O/'audit.json').write_text(json.dumps(audit,indent=2))
if len(payload)==5:
 ...
 bpy.ops.wm.save_as_mainfile(filepath=str(O/'HELD-diagnostic.blend'))
print('185 FINAL',json.dumps(audit),flush=True)
