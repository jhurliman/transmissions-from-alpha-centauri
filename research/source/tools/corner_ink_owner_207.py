...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/corner-ink-207';O.mkdir(parents=True,exist_ok=True);bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/scene-completion-205/scene.blend'));s=bpy.context.scene
for ob in s.objects:
...
for inst in dg.object_instances:
 ob=inst.object
 if ob.type!='MESH' or ob.name.startswith('189 Scene-wide fastener rust films'):continue
 ...
 ob.to_mesh_clear()
report=dict(source='205',roi=roi,ray_summary=dict(collections.Counter(r['object']for r in rows)),rays=rows,candidate_objects=bounds,edges=edges)
(O/'ownership.json').write_text(json.dumps(report,indent=2));print('SUMMARY',report['ray_summary'],'EDGES',len(edges),'OBJECTS',list(set(e['object']for e in edges)),flush=True)
