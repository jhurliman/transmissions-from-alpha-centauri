...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/studies/coliseum-154/mapping';bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-152/scene.blend'));s=bpy.context.scene;cam=s.camera;W,H=3840,2885
...
def ray(x,y):
 ...
 return rec
regions={'central_crown':[1930,435,2085,605]}
result={'source':'art/studies/coliseum-152/scene.blend','resolution':[W,H],'spacing_px':4,'regions':{},'objects':{}}
...
names=[name for name,count in counts.items()if name and name.startswith('COL110 U')and ('fractured upper wall'in name or 'aperture head'in name)]
for name in names:
