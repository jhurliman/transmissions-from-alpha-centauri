...
R=Path(__file__).resolve().parents[1];import os
O=R/'art/studies/coliseum-115'/os.environ.get('COLISEUM_VARIANT','A')
...
def snapshot(path):
 ...
 for o in s.objects:
  ...
  if hasattr(o.data,'materials'):
   ...
   for m in o.data.materials:
    if not m or m.name in mats or o.name=='Distant dust volume - real lighting':continue
 ...
 return data,mats
x,xm=snapshot(R/'art/studies/coliseum-114/scene.blend');y,ym=snapshot(O/'scene.blend')
r={'baseline':'114','candidate':'115','nonlandmark_objects_compared':len(x),'object_changes':[k for k in x if x[k]!=y.get(k)],'unexpected_objects':sorted(set(y)-set(x)),'materials_compared':len(xm),'material_graph_changes':[k for k in xm if xm[k]!=ym.get(k)]};r['authorized_changes']={'object_material_assignment':'Distant dust volume - real lighting','new_material':'115 Street haze height falloff'};r['unexpected_object_changes']=[k for k in r['object_changes'] if k!='Distant dust volume - real lighting'];r['unexpected_material_changes']=[k for k in r['material_graph_changes'] if k not in {'Distant atmospheric dust','Distant dust'}];(O/'preservation.json').write_text(json.dumps(r,indent=2));print(r)
