...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/alley-weathering-146/combined';O.mkdir(parents=True,exist_ok=True)
...
d=json.loads((R/'art/studies/alley-weathering-145/geometry/audit.json').read_text());specs=[];panelmat=make_panel_material(panel_regions=d['panels'])
for p in d['panels']:
