...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-113'
from coliseum_collapse_113 import collapse_landmark
from coliseum_materials_113 import apply_materials
...
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-112/scene.blend'));s=bpy.context.scene;C=bpy.data.collections['110 Coliseum detailed front ruin']
(O/'collapse-audit.json').write_text(json.dumps(collapse_landmark(C),indent=2));apply_materials(C)
from coliseum_link_113 import link_intact_additions
(O/'linked-kit-audit.json').write_text(json.dumps(link_intact_additions(C),indent=2))
