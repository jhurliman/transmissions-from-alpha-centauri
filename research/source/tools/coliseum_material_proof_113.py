...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-113/material-study';O.mkdir(parents=True,exist_ok=True)
from coliseum_materials_113 import apply_materials
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-112/scene.blend'));s=bpy.context.scene;C=bpy.data.collections['110 Coliseum detailed front ruin'];apply_materials(C)
