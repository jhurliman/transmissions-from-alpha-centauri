"""Read-only fresh163scene/kit geometry, normals, materials and dependency audit."""
import bpy,json,hashlib,array
from pathlib import Path
R=Path.cwd();O=R/'art/studies/coliseum-163'
def ids():
 rows=[]
...
A=read('art/studies/coliseum-163/scene.blend');B=read('art/studies/coliseum-163/kit.blend');out={}
for key,d in [('scene163',A),('kit163',B)]:out[key]={k:v for k,v in d.items()if k!='stamp'}
a=A['stamp'];b=B['stamp'];out['kit163_vs_scene163']={'expected_objects':len(a),'actual_objects':len(b),'missing':sorted(set(a)-set(b)),'extra':sorted(set(b)-set(a)),'changed':{k:[f for f,vv in v.items()if vv!=b.get(k,{}).get(f)]for k,v in a.items()if v!=b.get(k)}}
assert not out['kit163_vs_scene163']['missing'] and not out['kit163_vs_scene163']['extra'] and not out['kit163_vs_scene163']['changed']
