"""Read-only dense support comparison for the proposed left course/sill end simplification."""
import bpy
import json
import math
import sys
...
held = '--diagnostic' in sys.argv
candidate = R / ('art/studies/coliseum-187/geometry/HELD-diagnostic.blend' if held else 'art/studies/coliseum-187/geometry/candidate.blend')
...
for record in snap:
    for index, stroke in enumerate(record['strokes']):
        ...
        if tested:
            ...
            total += tested
result = {'source_digest': inventory['source_drawing_digest'], 'candidate': str(candidate.relative_to(R)), 'held_diagnostic_only': held,
...
(O / ('held-support-comparison.json' if held else 'candidate-support-comparison.json')).write_text(json.dumps(result, indent=2) + '\n')
