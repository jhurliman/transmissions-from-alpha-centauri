...
def buttress():
 ...
 d=2.50*math.tan(math.radians(30));pts=[(-d-.18,0),(-d+.18,0),(.18,2.50),(-.18,2.50)]
 ...
 box('Buttress head bearing',(0,0,2.50),(.43,.48,.08),steel)
register('buttress_30',buttress,{'kind':'solid_support','height':2.54,'rake':30,'foot':[0,-2.5*math.tan(math.radians(30)),0],'head':[0,0,2.50]})
...
def right():
 for x in [-1.412,1.412]:
  ...
  box('Ground frontage plinth',(x,.50,-.145),(2.8,.32,.33),concrete)
...
if '--right-only' in sys.argv:
 for col in proofs.values():col.hide_render=True;col.hide_viewport=True
 proofs={k:v for k,v in proofs.items() if k=='right_window_frontage'}
...
tail=tail.replace("for key,col in proofs.items():\n",'''for key,col in proofs.items():
 target=Vector((0,.1,2.65 if key!='canopy' else .85))
 cam.location=target+Vector((5,-11,4));cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler()
 d.ortho_scale=11.4 if key=='right_window_frontage' else (9.7 if key=='left_inclined_bay' else 6.3)
''')
exec(tail)
