...
def make_panel_material(name='146 Sun-responsive slate panel',origin=(0,0,0),across=(1,0,0),up=(0,0,1),span=(4.546,1.42),panel_regions=None):
 ...
 pattern=math('ADD',math('MULTIPLY',fleck.outputs['Fac'],.77),math('MULTIPLY',fray.outputs['Fac'],.23));islands=remap(pattern,.552,.603);holes=remap(pattern,.365,.455,1,0)
 retained=math('MULTIPLY',broad,math('SUBTRACT',1,math('MULTIPLY',math('MULTIPLY',holes,bottom),.42)))
 mask=math('MAXIMUM',retained,math('MULTIPLY',math('MULTIPLY',islands,bottom),.94));mask.node.label='146 Broad fields plus bottom-weighted fine splotches'
 ...
 body=mix(warm,(.155,.175,.235,1),(.34,.235,.18,1),'146 Sun-dependent cool-to-warm coating response');body=mix(1,body,illum,'146 Actual diffuse value','MULTIPLY');body=mix(.065,body,fine.outputs['Fac'],'146 Fine retained coating grain','MULTIPLY');sheen=math('MULTIPLY',mask,remap(dbw.outputs[0],.10,.70,0,.42));lift=mix(1,body,(1.38,1.38,1.38,1),'146 Broad-light sheen value','MULTIPLY');body=mix(sheen,body,lift,'146 Lightening response retains fine pattern in cool fill')
 ao=node('ShaderNodeAmbientOcclusion');ao.inputs['Distance'].default_value=.12;shadow=remap(ao.outputs['AO'],.15,.85,.60,1);body=mix(1,body,shadow,'146 Local physical recess shadow','MULTIPLY')
 em=node('ShaderNodeEmission','146 Surface color before optional weathering');l.new(body,em.inputs[0]);out=node('ShaderNodeOutputMaterial');l.new(em.outputs[0],out.inputs['Surface']);m['146 material']='Broad and fine bottom-weighted susceptibility with actual warm-incident-light and specular response; no unlit warm field';return m
