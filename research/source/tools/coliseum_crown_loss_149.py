...
def apply(C):
 ...
 for idx,name in enumerate(NAMES):
  ...
  profile=[(3.69,47.93),(3.62,49.03),(3.83,50.04),(3.74,51.10),(3.94,53.0),(6.95,53.0),(6.68,51.4),(6.81,50.50),(6.68,49.85),(6.22,49.95),(5.75,50.15),(5.05,49.92),(4.70,49.52),(4.51,48.12),(4.06,47.83)]
  vs=[]
  ...
  step=[(5.58,53.1),(6.48,53.1),(6.46,50.38),(5.94,50.20),(5.47,50.42)]
  ...
  bpy.data.objects.remove(tmp,do_unlink=True);ob['149 crown loss']='Under-crown inward loss; loss opens through front cap; rear skyline retained';out.append({'object':name,'before':before,'after':after,'new_vertices_world':changed,'retained_old_vertices':sum(tuple(v.co)in retained for v in me.vertices),'original_vertex_count':len(src),'outward_added_geometry':False,'existing_materials_preserved':True,'new_core_original_bounds':[[min(aa.data[v.index].vector[k]for v in me.vertices if tuple(v.co)not in retained),max(aa.data[v.index].vector[k]for v in me.vertices if tuple(v.co)not in retained)]for k in range(3)]})
