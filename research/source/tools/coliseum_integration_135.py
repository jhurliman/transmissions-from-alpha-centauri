"""Integrate broader connected crown reconstruction and attached broken-cornice weathering."""
import bpy,sys,json,time,hashlib,array
from pathlib import Path
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-135';O.mkdir(exist_ok=True)
def fingerprint(s):
 out={}
 for ob in s.objects:
...
if 'render' not in sys.argv:
 bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-134/scene.blend'));s=bpy.context.scene;before=fingerprint(s);C=next(c for c in s.collection.children_recursive if c.name=='110 Coliseum detailed front ruin' and not c.library)
 from coliseum_fracture_135 import apply as fracture
 from coliseum_weathering_135 import apply as weather
 t=time.time();audit={'fracture':fracture(C),'weathering':weather(C,1.0)};audit['seconds']=time.time()-t
 after=fingerprint(s);changes={k:[f for f in v if v[f]!=after.get(k,{}).get(f)]for k,v in before.items() if v!=after.get(k)}
 allowed={'COL110 U4 fractured upper wall L','COL110 U4 fractured upper wall R','COL110 U5 fractured upper wall L'}
