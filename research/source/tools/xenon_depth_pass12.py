...
R=Path(__file__).resolve().parents[1];O=R/'art/reviews/xenon-012';O.mkdir(exist_ok=True)
...
C=bpy.data.collections.new('012 Long ruined city - depth layers');s.collection.children.link(C)
 ...
 for i in range(n):
  ...
  if height<5:
   ...
   for level in range(int(height/2.7)):
    ...
    if level%2==0:
     box('Layer %02d surviving wall bay'%layer,(x-width*.25,y,z+.7),(width*.5,.32,.8),base)
