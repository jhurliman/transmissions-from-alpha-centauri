"""Combine retained subtractive crown151 with facade-age152 on baseline150."""
import bpy,sys,json,time,hashlib,array
from pathlib import Path
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-152';O.mkdir(exist_ok=True)
...
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-150/scene.blend'));s=bpy.context.scene;before=fingerprint(s);mats=material_snapshot();C=next(c for c in bpy.data.collections if c.name=='110 Coliseum detailed front ruin'and not c.library)
from coliseum_crown_thickness_151 import apply as geometry
from coliseum_crown_material_151 import apply as core
from coliseum_facade_age_152 import apply as age
result={'crown_geometry':geometry(C),'crown_core':core(C),'facade_age':age(C),'native_visibility':visibility(s,embed=True)};after=fingerprint(s);ma=material_snapshot();changes={k:[f for f in v if v[f]!=after[k][f]]for k,v in before.items()if v!=after[k]};geo={r['object']for r in result['crown_geometry']['targets']};mat={r['object']for r in result['crown_core']['assignments']+result['facade_age']['assignments']}
