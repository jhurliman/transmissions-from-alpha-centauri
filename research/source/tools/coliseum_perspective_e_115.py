...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-perspective-115/E';O.mkdir(parents=True,exist_ok=True)
...
objects=list(C.objects);count=0;min_y=1e9;max_shift=0;bad=0;baseline_bad=0;new_slivers=[]
for ob in objects:
 ...
 for v in mesh.vertices:
  ...
  if rad<outer:rad=outer+.55*(rad-outer)
 ...
 for poly in mesh.polygons:
  if poly.area<1e-12:
   bad+=1
   if old.polygons[poly.index].area>=1e-12:new_slivers.append([ob.name,poly.index,old.polygons[poly.index].area,poly.area])
