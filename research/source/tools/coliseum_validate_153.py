...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-153';a=Image.open(R/'art/studies/coliseum-152/main-4k.png').convert('RGB');b=Image.open(O/'main-4k.png').convert('RGB');aa=np.array(a).astype('int16');bb=np.array(b).astype('int16');assert aa.shape==bb.shape==(2885,3840,3)
...
r={'baseline':'152main-4k.png','native_size':[3840,2885],'landmark_region':[1250,300,2500,1300],'outside_landmark_pixels_over4':int(((d>4)&outside).sum()),'outside_landmark_max_channel_delta':int(d[outside].max()),'outside_regions':rows,'note':'Inspect all meaningful outside regions; geometry/data preservation does not imply identical Freestyle raster.'}
for name,box in [('crown',(1430,435,1645,630)),('primary',(1890,565,1980,650)),('secondary',(2110,585,2210,680))]:
