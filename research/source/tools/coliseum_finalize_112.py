...
R=Path(__file__).resolve().parents[1];sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-112'
from coliseum_fracture_112 import fracture_surfaces
...
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-111/scene.blend'));s=bpy.context.scene;C=bpy.data.collections['110 Coliseum detailed front ruin']
(O/'fracture-audit.json').write_text(json.dumps(fracture_surfaces(C),indent=2));apply_materials(C)
