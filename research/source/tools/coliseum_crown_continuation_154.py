...
PROFILE=[(5.15,49.40),(6.35,49.26),(6.70,49.45),(7.18,49.05),(7.70,48.75),(8.45,48.70),(8.70,48.42),(10.18,48.37),(10.65,48.70),(11.20,48.90),(11.20,54.),(7.70,54.),(5.15,54.)]
...
def apply(C):
    ...
    for name in TARGETS:
        ...
        def cut(profile,depth,slope=.465):
            # Distinct central profile; its low edge stays above existing frame tops.
            vv=[iv@Vector((x,195.95+.465*x+.09*(z-49.5)+(depth+((slope[0]if x<7.70 else slope[1])-.465)*(x-7.70) if back else -4.),z))for back in [False,True]for x,z in profile];n=len(profile)
            k=next(i for i,(x,z)in enumerate(profile)if abs(x-7.70)<1e-8 and z<54);mid=next(i for i,(x,z)in enumerate(profile)if abs(x-7.70)<1e-8 and z==54)
            left=list(range(k+1))+[mid,n-1];right=list(range(k,mid+1))
            ff=[tuple(range(n-1,-1,-1)),tuple(i+n for i in left),tuple(i+n for i in right)]+[(k,(k+1)%n,(k+1)%n+n,k+n)for k in range(n)]
        ...
        cut(PROFILE,2.40,(1.132,-.021))
        # Two unequal retained planes open through the original crest.
        print('154 primary cut',name,topology(tmp),flush=True)
        # One watertight cutter avoids adjacent-cutter numerical seam slivers.
    ...
    return {'source':'152','targets':rows,'operation':'DIFFERENCE only','inherited_R_geometry_caveat':'Five unchanged float64 crossing pairs in source lower R; zero new pairs allowed; legacy float32 test was not reliable under transformed temporary objects','profile_world_xz':PROFILE,'rear_plane':'V2 two oblique planes meet atX7.70; depth2.40 at center, slopes1.132 and-.021; startsX5.15 preserving left ridge','references':['UCL-01','UCL-02','DP-03'],'no_constructive_additions':True}
