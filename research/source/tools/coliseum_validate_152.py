...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-152';a=Image.open(R/'art/studies/coliseum-150/main-4k.png').convert('RGB');b=Image.open(O/'main-4k.png').convert('RGB');aa=np.array(a).astype('int16');bb=np.array(b).astype('int16');assert aa.shape==bb.shape==(2885,3840,3)
...
r={'baseline':'150main-4k.png','native_size':[3840,2885],'landmark_region':[1250,300,2500,1300],'outside_landmark_pixels_over4':int(((d>4)&outside).sum()),'outside_landmark_max_channel_delta':int(d[outside].max()),'outside_regions':rows,'note':'Inspect all meaningful outside regions; geometry/data preservation does not imply identical Freestyle raster.'}
for name,box in [('crown',(1430,435,1645,630)),('primary',(1665,520,1940,785)),('secondary',(2065,540,2300,790))]:
