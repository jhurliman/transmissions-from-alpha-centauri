...
R=Path(__file__).resolve().parents[1];O=R/'art/studies/coliseum-127/fracture';sys.path.insert(0,str(R/'tools'))
...
def apply(C):
 ...
 for index,(center,t,r,z,size,kind)in enumerate(candidates):
  ...
  old=ob.data;before=topology(ob);oldtree,oldvs,_=tree(ob);ob.data=old.copy();mod=ob.modifiers.new('127 negative fracture','BOOLEAN');mod.operation='DIFFERENCE';mod.solver='MANIFOLD';mod.object=cut;bpy.context.view_layer.objects.active=ob
  try:
   bpy.ops.object.modifier_apply(modifier=mod.name);after=topology(ob)
