...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/reviews/xenon-067';O.mkdir(exist_ok=True)
proofs=json.loads((R/'art/reviews/xenon-061/validation.json').read_text())
for label,opening in [('A',.20),('B',.40),('C',.60)]:
 maximum=1.0
 ...
 for hostName,a in zip(['Architecture | buttress_45','Front-left section instance'],proofs):
  ...
  ramp=nt.nodes.new('ShaderNodeMapRange');ramp.label='067 Depth darkness | '+label;ramp.clamp=True;nt.links.new(dot.outputs['Value'],ramp.inputs['Value']);ramp.inputs['From Min'].default_value=0;ramp.inputs['From Max'].default_value=depth;ramp.inputs['To Min'].default_value=1-opening;ramp.inputs['To Max'].default_value=1-maximum
