...
for ob in C.all_objects:
 ...
 for i,slot in enumerate(ob.material_slots):
  ...
  if old not in copies:
   ...
   em=n.new('ShaderNodeEmission');em.label='175 Direct diagnostic surface output';l.new(signals.outputs[0],em.inputs[0]);output=next(q for q in n if q.type=='OUTPUT_MATERIAL' and q.is_active_output);l.new(em.outputs[0],output.inputs['Surface']);copies[old]=m
