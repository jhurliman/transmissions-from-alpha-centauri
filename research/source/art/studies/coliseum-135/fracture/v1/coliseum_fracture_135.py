...
def _one(C,name):
 ...
 def top(u,r):
  ...
  inset=(.14+.025*math.sin(u*3.9)+.015*math.sin(u*12.7))*end*smooth(73.65,75,r)
 ...
 for level in [0,1,2]:
  ...
  for p in domain:
   ...
   zz=max(cutz+.10,h-[1.10,.65,.30][level]);rr=r+(top(u,r)[0]-r)*[0.,.50,.82][level]-([0.,1.,.42][level]*notch)
