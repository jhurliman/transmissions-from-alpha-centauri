...
def apply(scene):
 ...
 for name,origin,radii,normal,group in SITES:
  ...
  for sl in ob.material_slots:
   ...
   gate=calc('MULTIPLY',spatial,smooth(dot.outputs['Value'],.80,.97));changes=[]
   strengths={'Fine stone pits and dry pigment':.38,'Connected worn pigment islands':.22,'127 Broken sponge pigment at 4K scale':.30}
   for q in list(n):
    ...
    if group=='C1'and q.type=='MIX_RGB'and q.label in ['135 Deposits fed by broken cornice','148 Connected sheltered oxide and dust']:
     original=q.inputs[0].links[0].from_socket if q.inputs[0].is_linked else q.inputs[0].default_value;strength=.30 if q.label.startswith('135')else.22;fac=calc('MULTIPLY',original,calc('SUBTRACT',1,calc('MULTIPLY',gate,strength)));l.new(fac,q.inputs[0]);changes.append({'node':q.name,'label':q.label,'maximum_factor_reduction':strength})
