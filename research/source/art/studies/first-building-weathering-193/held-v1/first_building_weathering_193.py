...
def finish(base,role,severity):
 m=base.copy();m.name='193 Facade40 '+role+' '+base.name;m['193 scope']='First-left facade and rear portal jamb only';m['193 severity']=severity
 ...
 geo=node('ShaderNodeNewGeometry','world-continuous weather field');sep=node('ShaderNodeSeparateXYZ','world position');l.new(geo.outputs['Position'],sep.inputs[0]);across=sep.outputs['Y'];up=sep.outputs['Z']
 ...
 position=vector(across,up,2.13)
 # Broad connected susceptibility remains quiet; details break it up near real panel edges/bases.
 broad=noise(vector(math('MULTIPLY',across,.37),math('MULTIPLY',up,.22),3.4),1,2.4)
 ...
 medium=noise(vector(math('MULTIPLY',across,1.15),math('MULTIPLY',up,.80),6.1),5.4,2.7)
 ...
 rain=noise(vector(math('MULTIPLY',across,15),math('MULTIPLY',up,.24),9.37),1,1.7)
 ...
 l.new(body,em.inputs[0]);m['193 finish']='Continuous multi-panel field + Generated panel-base/edge weighting + world-Z rain taper; inherits current lit source color';return m
