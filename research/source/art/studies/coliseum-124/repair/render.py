...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/studies/coliseum-124/repair';sys.path.insert(0,str(R/'tools'));from coliseum_r_normalize_124 import apply
...
for label,file in [('before','baseline.blend'),('after','geometry.blend')]:
 bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-123/chips/baseline.blend'));
 if label=='after':apply(bpy.data.collections['110 Coliseum detailed front ruin'])
 s=bpy.context.scene;s.render.threads_mode='FIXED';s.render.threads=4;s.render.use_freestyle=False;s.render.resolution_percentage=100
