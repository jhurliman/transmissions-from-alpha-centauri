...
def apply(C):
 start=time.time();ob=next(o for o in C.objects if o.name==TARGET);old=ob.data;before=topology(ob);evaluated_before=evaluated_topology(ob);original,world,unpack=mapping();iv=ob.matrix_world.inverted();ac=-math.pi+9.5*math.tau/36
 ...
 f=eligible[0];cap_indices=set(f.vertices);original_coords=[v.co.copy()for v in old.vertices]
 ...
 for v in vertices:
  r,a,z=unpack(ob.matrix_world@v.co);u=(a-ac)*75
 ...
 for v in me.vertices:
  if tuple(round(x,6)for x in v.co)not in old_positions:
   r,a,z=unpack(ob.matrix_world@v.co);attr.data[v.index].vector=original(r,a,z)
