...
def mineral(name,base):
 m=bpy.data.materials.new(name);m.use_nodes=True;n=m.node_tree.nodes;l=m.node_tree.links;n.clear();d=n.new('ShaderNodeBsdfDiffuse');d.inputs[0].default_value=(1,1,1,1);d.inputs['Roughness'].default_value=.7;sr=n.new('ShaderNodeShaderToRGB');l.new(d.outputs[0],sr.inputs[0]);bw=n.new('ShaderNodeRGBToBW');l.new(sr.outputs[0],bw.inputs[0]);r=n.new('ShaderNodeValToRGB');r.color_ramp.interpolation='LINEAR';cols=[(.18,[v*.46 for v in base]),(.52,[v*.7 for v in base]),(.85,base),(1.08,[min(255,v*1.34) for v in base])]
 for i,(p,c) in enumerate(cols):
  e=r.color_ramp.elements[i] if i<2 else r.color_ramp.elements.new(p);e.position=p;e.color=rgb(c)
 l.new(bw.outputs[0],r.inputs[0]);gl=n.new('ShaderNodeBsdfGlossy');gl.inputs['Roughness'].default_value=.36;gr=n.new('ShaderNodeShaderToRGB');l.new(gl.outputs[0],gr.inputs[0]);gb=n.new('ShaderNodeRGBToBW');l.new(gr.outputs[0],gb.inputs[0]);g=n.new('ShaderNodeMapRange');g.clamp=True;g.inputs['From Min'].default_value=.22;g.inputs['From Max'].default_value=1.8;g.inputs['To Max'].default_value=.42;l.new(gb.outputs[0],g.inputs[0]);mix=n.new('ShaderNodeMixRGB');l.new(g.outputs[0],mix.inputs[0]);l.new(r.outputs[0],mix.inputs[1]);mix.inputs[2].default_value=rgb((142,113,81));e=n.new('ShaderNodeEmission');l.new(mix.outputs[0],e.inputs[0]);out=n.new('ShaderNodeOutputMaterial');l.new(e.outputs[0],out.inputs[0]);return m
...
def apply(s):
 ...
 materials=[mineral('085 earth mineral '+str(i),c) for i,c in enumerate([(78,56,41),(85,61,43),(64,47,37),(99,73,51),(73,56,47)])]
 ...
 for i in range(17500):
  ...
  if not hit:continue
  z=loc.z
  u=rng.random();r=rng.uniform(.013,.038) if u<.7 else rng.uniform(.038,.085) if u<.96 else rng.uniform(.085,.13)
  h=r*rng.uniform(.10,.38);N=rng.choice([4,5,6]);angle=rng.random()*math.tau;elong=rng.uniform(.6,2.1);cidx=rng.choices(range(5),[4,2,3,1,2])[0];start=len(verts)
  for j in range(N):
