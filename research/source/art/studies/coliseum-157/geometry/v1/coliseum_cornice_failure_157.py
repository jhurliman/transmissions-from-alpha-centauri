    ...
    for name in TARGETS:
        ...
        if 'profile' in name:
            layer=int(name.rsplit('profile',1)[1]);spec={
                0:(18.3,19.0,20.3,24.7,2.20),
                1:(17.4,18.7,22.1,26.0,.00),
                2:(18.0,19.2,21.2,25.2,1.05),
                3:(17.1,18.2,22.7,27.3,-1.03),
                4:(17.9,19.0,21.6,25.0,2.20)}[layer]
            xs=list(spec[:4]);offset=spec[4]
            heights=[((M@v.co).z)for v in me.vertices];lows=[min(heights)-.05]*4;highs=[max(heights)+.05]*4
            depths=[.15,6.7,6.0,.10]if layer==3 else[.15,5.2,4.7,.10]
        else:
            xs=[17.9,19.2,22.0,25.1];offset=4.5
            lows=[41.20]*4;highs=[41.68,43.68,43.22,41.77];depths=[.08,.95,1.1,.05]
        verts=[]
