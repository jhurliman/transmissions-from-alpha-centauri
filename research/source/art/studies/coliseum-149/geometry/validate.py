...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/studies/coliseum-149/geometry';names=['COL110 U9 fractured upper wall L'];src={};trees={}
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-148/scene.blend'));dg=bpy.context.evaluated_depsgraph_get()
...
for name in names:
 ...
 while remain:
  ...
  comp.append(count)
 out.append({'object':name,'components':comp,'max_outside_source_signed_distance_m':max(positive,default=0),'source_coordinate_exact_retained_vertices':sum(tuple(v)in src[name]for v in vs),'new_face_core_count':sum(d.value>.5 for d in ob.data.attributes['149 Exposed crown core'].data)})
