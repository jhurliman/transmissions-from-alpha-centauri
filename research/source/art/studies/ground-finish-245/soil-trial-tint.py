...
def apply(scene,strength=.80):
 ...
 distance=op('SUBTRACT',8.3,x);reach=p.remap(distance,.0,4.6,1,0)
 ...
 broken=p.remap(op('ADD',op('MULTIPLY',coarse,.62),op('MULTIPLY',grit,.38)),.44,.65)
 ...
 tint=p.mix(1,original,(2.10,1.91,1.50,1),'245 soil-colored original grain','MULTIPLY')
 ...
 return {'strength':strength,'extent_xy':[3.7,9.3,-1.4,6.5],'original_texture_unchanged':True,'approach':'unequal curved broken trails plus loose granular fan','material':m.name}
