...
bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-131/scene.blend'));s=bpy.context.scene;s.render.threads_mode='FIXED';s.render.threads=4;s.render.resolution_x=1440;s.render.resolution_y=1082;s.render.resolution_percentage=100;s.render.use_border=False;s.render.use_crop_to_border=False
...
# Independent current-geometry proof: primary native Freestyle, no stale linked overlay.
s.render.use_compositing=False
for ls in s.view_layers[0].freestyle_settings.linesets:ls.show_render=True
s.render.filepath=str(O/'after.png');bpy.ops.render.render(write_still=True)
C=bpy.data.collections[a['collection']]
