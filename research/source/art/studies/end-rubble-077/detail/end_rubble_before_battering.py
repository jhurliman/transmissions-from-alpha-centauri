...
def apply_detail(scene):
 ...
 def beam(a,b,w,mat):
  ...
  for part,(yy,zz,wy,wz) in enumerate([(0,0,w,.045),(0,w*.46,w,.045),(-w*.46,w*.23,.045,w*.5)]):
   vs=[(x,y,z) for x in [0,L] for y in [yy-wy/2,yy+wy/2] for z in [zz-wz/2,zz+wz/2]]
   for j in range(4,8):vs[j]=(vs[j][0]+rng.uniform(-.10,.10),vs[j][1],vs[j][2])
   ob=mesh('078 severed channel '+str(part),vs,[(0,1,3,2),(4,6,7,5),(0,4,5,1),(2,3,7,6),(0,2,6,4),(1,5,7,3)],mat);ob.location=a;ob.rotation_euler=v.to_track_quat('X','Z').to_euler()
