...
def make_panel_material(name='145 Sun-responsive slate panel',origin=(0,0,0),across=(1,0,0),up=(0,0,1),span=(4.546,1.42)):
 ...
 fine=noise(pos,85,2);edge=noise(pos,24,1.5);U=math('DIVIDE',u,span[0]);V=math('DIVIDE',v,span[1])
 def region(points):
  ...
  signed=math('ADD',signed,math('MULTIPLY',math('SUBTRACT',edge.outputs['Fac'],.5),.018))
 ...
 rough=remap(mask,0,1,.83,.42);diff=node('ShaderNodeBsdfDiffuse');diff.inputs['Color'].default_value=(.75,.75,.75,1);drgb=node('ShaderNodeShaderToRGB');l.new(diff.outputs[0],drgb.inputs[0]);dbw=node('ShaderNodeRGBToBW');l.new(drgb.outputs[0],dbw.inputs[0]);illum=remap(dbw.outputs[0],.02,1.2,.42,1.1)
