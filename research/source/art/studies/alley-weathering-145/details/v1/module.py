...
def node_group(spec,anchors,seed):
 g=bpy.data.node_groups.new('145 Corrosion '+spec['id'],'ShaderNodeTree');g.interface.new_socket(name='Position',in_out='INPUT',socket_type='NodeSocketVector');g.interface.new_socket(name='Strength',in_out='INPUT',socket_type='NodeSocketFloat').default_value=1;g.interface.new_socket(name='Mask',in_out='OUTPUT',socket_type='NodeSocketFloat');g.interface.new_socket(name='Tint multiplier',in_out='OUTPUT',socket_type='NodeSocketColor');n,l=g.nodes,g.links;gi=n.new('NodeGroupInput');go=n.new('NodeGroupOutput')
 def calc(op,*args):
 ...
 noise=n.new('ShaderNodeTexNoise');noise.inputs['Scale'].default_value=70;noise.inputs['Detail'].default_value=2;l.new(pos,noise.inputs['Vector']);jitter=calc('MULTIPLY',calc('SUBTRACT',noise.outputs['Fac'],.5),.22);field=0
 for a in anchors:
  du=calc('SUBTRACT',U,a['uv'][0]);down=calc('SUBTRACT',a['uv'][1],V)
  if a['kind']=='bolt':
   dist=calc('ADD',calc('SQRT',calc('ADD',calc('POWER',calc('DIVIDE',du,a['radius']),2),calc('POWER',calc('DIVIDE',down,a['radius']*1.08),2))),calc('MULTIPLY',calc('SUBTRACT',noise.outputs['Fac'],.5),.40));core=calc('SUBTRACT',1.05,calc('MULTIPLY',calc('MAXIMUM',0,calc('SUBTRACT',dist,.55)),2))
  else:core=0
  drift=calc('MULTIPLY',calc('SUBTRACT',noise.outputs['Fac'],.5),.012);cross=calc('DIVIDE',calc('ABSOLUTE',calc('ADD',du,drift)),a['width']);along=calc('DIVIDE',down,a['length']);fall=calc('MULTIPLY',calc('SUBTRACT',.70,calc('MAXIMUM',cross,along)),calc('GREATER_THAN',down,0));f=calc('MAXIMUM',core,fall);field=calc('MAXIMUM',field,f)
 gate=calc('LESS_THAN',calc('ABSOLUTE',depth),.045);finite=calc('GREATER_THAN',field,.01);field=calc('MULTIPLY',calc('MULTIPLY',calc('ADD',field,jitter),finite),gate);mask=calc('MULTIPLY',calc('MINIMUM',1,calc('MAXIMUM',0,calc('MULTIPLY',field,6))),gi.outputs['Strength']);l.new(mask,go.inputs['Mask'])
 ramp=n.new('ShaderNodeValToRGB');r=ramp.color_ramp;r.interpolation='CONSTANT';r.elements[0].position=.05;r.elements[0].color=(1.20,.62,.30,1);r.elements[1].position=.72;r.elements[1].color=(.16,.08,.10,1);r.elements.new(.32).color=(.63,.27,.20,1);r.elements.new(.52).color=(.35,.18,.23,1);l.new(field,ramp.inputs[0]);l.new(ramp.outputs[0],go.inputs['Tint multiplier']);g['145 anchors_json']=json.dumps(anchors);g['145 seed']=seed;return g
...
def attach_overlay(material,group):
 ...
 g=n.new('ShaderNodeGroup');g.node_tree=group;g.label='145 Disable corrosion with Strength0';geo=n.new('ShaderNodeNewGeometry');l.new(geo.outputs['Position'],g.inputs['Position']);mix=n.new('ShaderNodeMixRGB');mix.blend_type='MULTIPLY';l.new(g.outputs['Mask'],mix.inputs[0]);l.new(base,mix.inputs[1]);l.new(g.outputs['Tint multiplier'],mix.inputs[2]);l.new(mix.outputs[0],target);m['145 detail_layer']=True;return m
...
def apply_panels(specs,parent_collection=None,seed=145,occupancy=.70,add_nicks=False,install_material=True):
 ...
 for s in specs:
  ...
  for i,(sid,p)in enumerate(candidates):
   ...
   a={'kind':'bolt','uv':p,'world':list(world(p)),'radius':rng.uniform(.037,.049),'width':rng.uniform(.007,.015),'length':rng.uniform(.16,.60)};anchors.append(a);bolts.append(p)
  ...
  vs=[];fs=[];count=0;area=s['width']*s['height'];limit=round(area*24)
  for i in range(limit*3):
   if count>=limit:break
   p=(rng.uniform(.025,s['width']-.025),rng.uniform(.025,s['height']-.025))
   if not valid(p,s)or any(math.dist(p,b)<.075 for b in bolts):continue
   radius=rng.uniform(.0018,.0055);ry=radius*rng.uniform(.55,1.7);start=len(vs);vs.extend(world((p[0]+radius*math.cos(k*math.tau/5),p[1]+ry*math.sin(k*math.tau/5)),.0006)for k in range(5));fs.append(tuple(range(start,start+5)));count+=1
