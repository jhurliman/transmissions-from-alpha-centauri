...
TARGETS=[('COL110 T2 band08 profile1',0,.11,0),('COL110 T2 band08 profile2',0,.075,1),('COL110 T2 band09 profile2',0,.10,2)]
def apply(C):
 ...
 for name,index,depth,variant in TARGETS:
  ...
  ring=[(.16+.62*u,.1+.8*v)for u,v in ring]
 ...
 return {'references':['UCL-01','UCL-02','DP-03'],'source':'140 native scene','targets':out,'scope':'Three inward-only exposed underside spalls on one existing cornice run; all other object geometry unchanged','production_integrated':False}
