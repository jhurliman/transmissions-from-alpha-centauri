 ...
 for p in plates:
  ...
  for b in bolts:
   ...
   for line in range(count):
    offset=b['r']*(rng.uniform(-.10,.10)if count==1 else [-.38,.35][line]+rng.uniform(-.06,.06));root=b['p']+across*offset+down*(b['r']*.80);start_v=(root-p['p']).dot(down);remaining=bottom-start_v-.0012
