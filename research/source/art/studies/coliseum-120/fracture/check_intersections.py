...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/studies/coliseum-120/fracture';bpy.ops.wm.open_mainfile(filepath=str(O/'proof.blend'));rows=[]
for ob in bpy.data.objects:
 if not ob.get('120 fracture detailed'):continue
...
(O/'intersection-check.json').write_text(json.dumps(rows,indent=2));print(rows)
