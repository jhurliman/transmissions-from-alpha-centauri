import bpy,bmesh,json,sys,time,types,numpy as np
...
for name in names:
 ...
 row['smallest_float64_source_triangles']=sorted([{'face':p.index,'area':float(np.linalg.norm(np.cross(np.array(verts[p.vertices[1]])-np.array(verts[p.vertices[0]]),np.array(verts[p.vertices[2]])-np.array(verts[p.vertices[0]])))/2),'vertices':list(p.vertices)}for p in src.polygons],key=lambda x:x['area'])[:4]
 ...
 for dd in row['crossing_details']:
  dd['source_roles']=[]
  for ti in dd['pair']:
   tt=me.loop_triangles[ti]; pp=me.polygons[tt.polygon_index];kk=tuple(sorted(tuple(me.vertices[i].co)for i in pp.vertices));dd['source_roles'].append({'triangle':ti,'polygon':pp.index,'polygon_vertices':list(pp.vertices),'triangle_vertices':list(tt.vertices),'source_exact':kk in keys,'cutter_distances':[ctree.find_nearest(me.vertices[i].co)[3]for i in tt.vertices],'source_distances':[tree.find_nearest(me.vertices[i].co)[3]for i in tt.vertices]})
