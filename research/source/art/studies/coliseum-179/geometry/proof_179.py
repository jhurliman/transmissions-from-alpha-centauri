...
import bpy,json,time,re,sys
from pathlib import Path
R=Path(__file__).resolve().parents[4];O=R/'art/studies/coliseum-179/geometry';rows=[]
for variant,path in [('before',R/'art/studies/coliseum-173/scene.blend'),('after',O/'candidate.blend')]:
 for mode in ['painted','clay']:
  ...
  if variant=='before':
   sys.path[:0]=[str(R/'tools'),str(R/'art/studies/coliseum-174/native')];from repair_174 import apply;apply(C)
  ...
  if mode=='clay':
   m=bpy.data.materials.new('179 Diagnostic clay');m.use_nodes=True;bs=m.node_tree.nodes.get('Principled BSDF');bs.inputs['Base Color'].default_value=(.42,.42,.42,1);bs.inputs['Roughness'].default_value=.8
...
(O/'proof-audit.json').write_text(json.dumps({'crop':[1440,440,1650,635],'Freestyle':False,'all_native_ink_preserved':True,'lighting_camera_world_haze':'Unchanged actual173 in both modes; clay replaces only non-ink landmark mesh slots','no_canonical_save':True,'rows':rows},indent=2))
