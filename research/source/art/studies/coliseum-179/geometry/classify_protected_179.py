...
for name in names:
 ...
 after=health(me,M);row['after']=after;row['removed_volume_m3']=before['volume']-after['volume'];newkeys={tuple(sorted(tuple(me.vertices[i].co)for i in p.vertices)):p.index for p in me.polygons};missing=[pi for pi in protectedids if tuple(sorted(tuple(src.vertices[i].co)for i in src.polygons[pi].vertices))not in newkeys];row['protected_missing']=missing;row['protected_missing_details']=[{'face':pi,'authored':[list(unpack(M@verts[i]))for i in src.polygons[pi].vertices],'source_vertices_exactly_present':[tuple(verts[i])in {tuple(v.co)for v in me.vertices}for i in src.polygons[pi].vertices]}for pi in missing]
