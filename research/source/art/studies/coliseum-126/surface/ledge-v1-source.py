 ...
 for ob in C.objects:
  ...
  for slot in ob.material_slots:
   ...
   if old not in cache:
    ...
    mask=calc('MULTIPLY',near,smooth(calc('ADD',branch,calc('MULTIPLY',calc('SUBTRACT',edge,.5),.42)),.40,.57));mask=calc('MULTIPLY',mask,strength)
    mix=node('ShaderNodeMixRGB','126 Broken mineral deposits below actual floor bands');mix.blend_type='MULTIPLY';l.new(mask,mix.inputs[0]);l.new(color,mix.inputs[1]);mix.inputs[2].default_value=(.40,.36,.48,1);color=mix.outputs[0]
