...
def apply(scene):
 ...
 for p,d in candidates:
  if all((p-q[0]).length>1.5 for q in chosen):chosen.append((p,d))
  if len(chosen)>=6:break
 bpy.context.view_layer.objects.active=ground
 for j,(p,d) in enumerate(chosen):
  r=rng.uniform(.17,.25);N=7;vs=[]
  for i in range(N):
   a=i*math.tau/N;rr=r*rng.uniform(.65,1.15);vs.append((p.x+rr*math.cos(a),p.y+rr*math.sin(a),-.035))
  vs.append((p.x+r*.18,p.y-r*.16,-.04-rng.uniform(.023,.043)));fs=[tuple(range(N-1,-1,-1))]+[(i,(i+1)%N,N) for i in range(N)]
