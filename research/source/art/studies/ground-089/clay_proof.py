...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');O=R/'art/studies/ground-089';bpy.ops.wm.open_mainfile(filepath=str(O/'scene.blend'));s=bpy.context.scene;s.render.threads_mode='FIXED';s.render.threads=4;s.render.use_freestyle=False;s.render.use_border=False
g=bpy.data.objects['Street foundation'];n=g.data.materials[0].node_tree
import json
(O/'material-audit.json').write_text(json.dumps([{'name':q.name,'type':q.type,'links':[(l.from_node.name,l.from_socket.name,l.to_node.name,l.to_socket.name) for l in n.links if l.to_node==q]} for q in n.nodes],indent=2))
...
for ob in s.objects:
 keep=ob.name=='Street foundation' 
...
s.use_nodes=False;s.render.filepath=str(O/'clay.png');bpy.ops.render.render(write_still=True)
