...
R=Path('/PATH/TO/transmissions-from-alpha-centauri');sys.path.insert(0,str(R/'tools'));O=R/'art/studies/coliseum-139/weathering';bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/scene-details-138/scene.blend'));s=bpy.context.scene
...
a,b,c=meshes(),mats(),obs();res=apply(C);aa,bb,cc=meshes(),mats(),obs();changes=[n for n in c if c[n]!=cc[n]];assert a==aa;assert all(bb[n]==v for n,v in b.items());assert all(n in {r['object']for r in res['assignments']}for n in changes)
(O/'preservation.json').write_text(json.dumps({'all_existing_mesh_geometry_and_corner_normals_exact':a==aa,'mesh_datablocks_checked':len(a),'all_existing_material_graphs_unchanged':True,'existing_materials_checked':len(b),'object_changes':changes,'only_selected_landmark_material_slots_changed':True,'camera_lights_transforms_untouched':True,'new_material_count':res['materials'],'limitations':'No geometry mutation; mesh hash covers positions, loop indices and corner normals, not every arbitrary attribute. Material graph hash covers nodes, input defaults and links; original materials not edited.'},indent=2));print('PASS139')
