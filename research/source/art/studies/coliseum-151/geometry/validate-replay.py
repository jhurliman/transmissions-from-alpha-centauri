...
O=R/'art/studies/coliseum-151/geometry';bpy.ops.wm.open_mainfile(filepath=str(R/'art/studies/coliseum-149/scene.blend'))
from coliseum_crown_thickness_151 import apply
C=next(c for c in bpy.data.collections if c.name=='110 Coliseum detailed front ruin'and not c.library);audit=apply(C);(O/'audit.json').write_text(json.dumps(audit,indent=2))
s=bpy.context.scene
for ob in s.objects:
