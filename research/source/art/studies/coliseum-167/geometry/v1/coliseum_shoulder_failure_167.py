...
PROFILE=[(-13.25,43.0),(-10.10,43.0),(-9.85,41.60),(-10.45,41.45),(-10.55,40.30),(-11.25,40.20),(-11.40,38.80),(-11.90,38.55),(-12.10,36.70),(-12.80,36.80),(-12.95,38.0),(-13.25,39.0)]
def health(me,M):
...
def apply(C):
 ...
 for name in names:
  ...
  if not world or max(v.x for v in world)<-13.25 or min(v.x for v in world)>-9.85 or max(v.z for v in world)<36.7 or min(v.z for v in world)>43 or min(v.y for v in world)>199.4:continue
  ...
  vv=[iv@Vector((x,175 if not back else 198.6+.11*(x+12)+.08*(z-40),z))for back in [False,True]for x,z in PROFILE];n=len(PROFILE);ff=[tuple(range(n-1,-1,-1)),tuple(range(n,n*2))]+[(i,(i+1)%n,(i+1)%n+n,i+n)for i in range(n)];cm=bpy.data.meshes.new('167 connected facing cutter');cm.from_pydata(vv,[],ff);bm=bmesh.new();bm.from_mesh(cm);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(cm);bm.free();co=bpy.data.objects.new('167 cutter',cm);C.objects.link(co)
  ...
  if not len(me.vertices):
   def inside(p):
    ...
    return hit and 175<p.y<198.6+.11*(p.x+12)+.08*(p.z-40)
   ...
   if contained and name.startswith('COL120 bay7 '):removals.append(ob);row['disposition']='Entire dentil lies within the same removed masonry volume; remove rather than leave floating decoration.'
  ...
  for p in src.polygons:
   ...
   if all(v.x< -13.25 or v.x> -9.85 or v.z<36.7 or v.z>43 or v.y>199.4 or v.y<175 for v in pts):
    # A face entirely beyond one same coordinate halfspace is definitely protected.
    if any(all(v[k]<lo for v in pts)or all(v[k]>hi for v in pts)for k,lo,hi in [(0,-13.25,-9.85),(1,175,199.4),(2,36.7,43)]):
 ...
 held=[r for r in rows if 'held'in r];audit={'source':'163','references':['UCL-01','UCL-02','DP-03'],'profile_world_xz':PROFILE,'depth':'Front175 to inclined rear198.6+.11(x+12)+.08(z-40)','targets':rows,'accepted_cpu':not held and bool(payload),'protected':'Tower7 core/silhouette, all arches and round columns remain untouched; only facing/collar/continuous-wall/sill targets listed.','seconds':time.time()-start}
 if not held:
