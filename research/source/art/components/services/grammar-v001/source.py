...
R=Path(__file__).resolve().parents[1];O=R/'art/components/services/grammar-v001';O.mkdir(parents=True,exist_ok=True)
...
def build(tree):
 for route in tree['children']:
  M=Matrix.Translation(Vector(route['root']))@(Matrix.Rotation(math.pi,4,'X') if route.get('down') else Matrix.Identity(4));q=None
  ...
  for level in [1.5,4.1,6.7]:
   if route['name']=='vent_feed' and level>6.5:continue
