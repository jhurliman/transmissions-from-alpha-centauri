...
for original in old.objects:
 ...
 if 'foot plate' not in ob.name.lower():ob.location.z+=1.30
# Extend the straight lower stems in assembly space; keep the original deck and Y joints rigid.
for x in [-1.20,1.20]:
 ob=box('Canopy lower stem extension',(x,-.90,.65),(.13,.15,1.30),steel)
 C.objects.unlink(ob);new.objects.link(ob)
 ...
 for i,y in enumerate(ys):
  ...
  if side>0:
   for row in range(6):
    ...
    if i in [3,8,12]:
     place('layout_transition',side,y,z)
     place('plate_square',side,y,z+2.20)
     place('floor_band',side,y,z+2.82)
    else:place(state,side,y,z)
...
for side,y,count in [(-1,5.4,12),(-1,16.6,9),(1,15.8,11)]:
 ...
 for z in [2+j*2.4 for j in range(count//2)]:
  box('Duct wall saddle',(side*7.98,y,z),(.38,.46,.10),steel)
