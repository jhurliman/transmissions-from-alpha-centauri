...
R=Path(__file__).resolve().parents[1];O=R/'art/reviews/xenon-028';O.mkdir(parents=True,exist_ok=True)
...
C=bpy.data.collections.new('028 Distant large service infrastructure');s.collection.children.link(C)
...
inst(wallturn('primary_bottom_wall_turn',.9,1.4,2.2,True),base(x,y,3.0));endpoints.append({'route':'primary','bottom':'90 into wall','top':'projecting building receiver'})
...
for zz in [4.2,8.2,12.1]:
 box('Primary structural standoff',(-7.8,y,zz),(1.70,.35,.22),steel,C)
...
for level,down in [(1.5,True),(z,False)]:inst(wallturn('companion_wall_'+str(down),.3,.6,1.0,down),base(x,y,level))
endpoints.append({'route':'medium companion','bottom':'90 into wall','top':'90 into wall'})
for zz in [2.0,5.0,8.0]:box('Companion standoff',(-8.20,y,zz),(.9,.18,.12),steel,C)
...
c=master('duct_building_plinth');box('Projecting service plinth',(0,.25,.35),(1.85,1.70,.70),paint,c);box('Plinth cover',(0,-.64,.36),(1.56,.06,.44),pale,c);inst('duct_building_plinth',base(x,y,0))
...
(O/'audit.json').write_text(json.dumps({'camera_preserved':True,'existing_mesh_geometry_unchanged':True,'unit_scale_instances':True,'hidden_old_services':hidden,'diameters_m':[.4,.6,1.8],'largest_housing_diameter_m':2.6,'duct_section_m':[1.4,1.0],'primary_alley_y':24.2,'primary_camera_forward_distance_m':38.2,'route_endpoints':endpoints,'unassigned_new_route_endpoints':0},indent=2))
