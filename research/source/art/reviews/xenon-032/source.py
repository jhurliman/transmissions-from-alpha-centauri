...
R=Path(__file__).resolve().parents[1];O=R/'art/reviews/xenon-032';O.mkdir(parents=True,exist_ok=True)
...
C=bpy.data.collections.new('032 Front left section architecture');s.collection.children.link(C)
kit=bpy.data.collections.new('FAC | front_left_section');kit.asset_mark();kit['part_id']='front_left_section';C.children.link(kit)
M=Matrix.Translation((-7.65,-3.8,0))@Matrix.Rotation(math.pi/2,4,'Z')
...
def mesh(n,vs,fs,m,transform=None):
 me=bpy.data.meshes.new(n);me.from_pydata(vs,[],fs);me.materials.append(m);ob=bpy.data.objects.new(n,me);kit.objects.link(ob);ob.matrix_world=M@(transform or Matrix.Identity(4));be=ob.modifiers.new('Manufactured edge','BEVEL');be.width=.012;be.segments=2;return ob
...
rake=math.radians(30);rise=2.4;run=rise*math.tan(rake);top_depth=run+.35
profile=[(.75,0),(.75,4.42),(0,4.42),(0,4.8),(run,7.2),(run,10.2),(top_depth,10.55),(top_depth,20),(3.8,20),(3.8,0)]
...
for u in [-3.65,3.65]:
 ...
 box('Column head capital',(u,.12,4.40),(1.14,.92,.24),pale)
# Continuous bearing belt and real horizontal underside of the projecting shoulder.
box('Shoulder underside',(0,.48,4.45),(8.4,.96,.10),paint)
for u0,u1 in [(-4.2,-1.4),(-1.4,1.4),(1.4,4.2)]:panel('Shoulder edge belt',u0,u1,4.45,4.8,-.015,pale)
# Sloped face in its own tangent frame: two deeply recessed slots within broad plate fields.
S=Matrix.Translation((0,0,4.8))@Matrix.Rotation(-rake,4,'X');L=rise/math.cos(rake)
...
for depth,z0,z1 in [(run,7.2,10.2),(top_depth,10.55,20)]:
 cuts=[z0]
...
T=Matrix.Translation((0,run,10.2))@Matrix.Rotation(-math.pi/4,4,'X')
...
kit['section_rules']=json.dumps({'foot_angle':15,'main_shoulder_angle':30,'upper_transition_angle':45,'recessed_base_depth':.75,'shoulder_rise':2.4,'shoulder_setback':run,'upper_setback':top_depth,'width':8.4,'height':20,'construction':'Connected extruded section; surface panels and reveals use tangent frames'})
